    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Indicativo de Leitura</h1>
        </div>
        ...
        <div class="margin-top-30">
          ...
          <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
            :value="frmIndLeituraTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="IL_CODIGO"
...
export default {
  ...
  methods: {
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.IL_CODIGO === data.IL_CODIGO,
      };
    },
    async fetchData() {
      this.isLoading = true;
      try {
        ...
        const response = await axios.get(`${baseUrl}/INDICATIVO_LEITURA_ALL`);
        this.frmIndLeituraTable = response.data; 
      } catch (error) {
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmIndLeituraTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
