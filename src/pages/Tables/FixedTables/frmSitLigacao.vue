    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Situação da Ligação</h1>
        </div>
        ...
        <div class="margin-top-30">
          ...
          <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
            :value="frmSitLigacaoTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="SIT_LIGACAO"
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {
          name: "DESC_SIT_LIGACAO",
          required: true,
          label: "Descrição",
          align: "left",
          field: "DESC_SIT_LIGACAO",
        },
      ],
      frmSitLigacaoTable: [],
      isLoading: false,
  ...
  methods: {
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.SIT_LIGACAO === data.SIT_LIGACAO,
      };
    },
    async fetchData() {
      this.isLoading = true;
      try {
        ...
        const response = await axios.get(`${baseUrl}/SITUACAO_LIGACAO_ALL`);
        this.frmSitLigacaoTable = response.data; 
      } catch (error) {
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmSitLigacaoTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
