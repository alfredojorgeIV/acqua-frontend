    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Finalidade da Unidade de Serviço</h1>
        </div>
        ...
        <div class="margin-top-30">
          ...
          <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
            :value="frmFinalidadeTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="FINALIDADE_DIST"
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        {
          name: "FINALIDADE_DIST",
          required: true,
          label: "Código",
          align: "left",
          field: "FINALIDADE_DIST",
        },
        {
          name: "DESC_FINALIDADE",
          required: true,
          label: "Descrição",
          align: "left",
          field: "DESC_FINALIDADE",
        },
      ],
      frmFinalidadeTable: [],
      isLoading: false,
  ...
  methods: {
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.FINALIDADE_DIST === data.FINALIDADE_DIST,
      };
    },
    async fetchData() {
      this.isLoading = true;
      try {
        ...
        const response = await axios.get(`${baseUrl}/FINALIDADE_DIST_ALL`);
        this.frmFinalidadeTable = response.data; 
      } catch (error) {
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmFinalidadeTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
