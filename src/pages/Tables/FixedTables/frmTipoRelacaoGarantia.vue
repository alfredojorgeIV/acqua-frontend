      ...
      <div class="content">
        <section>
          <div class="intro-services-to-be-billed">
            <h1 ref="title" class="title-page">Tipo de Relação dos Serviços de Garantia</h1>
          </div>
          ...
          <div class="margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmTipoRelacaoGarantiaTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="TIPO_RELACAO"
  ...
  export default {
    ...
    methods: {
      ...
      rowClass(data) {
        return {
          'pointer-cursor': true,
          'active-row': this.selectedRow && this.selectedRow.TIPO_RELACAO === data.TIPO_RELACAO,
        };
      },
      async fetchData() {
        this.isLoading = true;
        try {
          ...
          const response = await axios.get(`${baseUrl}/TIPO_RELACAO_GARANTIA_ALL`);
          this.frmTipoRelacaoGarantiaTable = response.data; 
        } catch (error) {
      ...
      printPDF() {
        ...
        doc.autoTable({
          ...
          body: this.frmTipoRelacaoGarantiaTable.map(row => this.columns.map(col => row[col.field])),
          theme: 'grid',
