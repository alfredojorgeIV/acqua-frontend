    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Tipo de Cobrança</h1>
        </div>
        ...
        <div class="margin-top-30">
          ...
          <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
            :value="frmTipoCobrancaTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="TIPO_COBRANCA"
...
export default {
  ...
  methods: {
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.TIPO_COBRANCA === data.TIPO_COBRANCA,
      };
    },
    async fetchData() {
      this.isLoading = true;
      try {
        ...
        const response = await axios.get(`${baseUrl}/TIPO_COBRANCA_ALL`);
        this.frmTipoCobrancaTable = response.data; 
      } catch (error) {
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmTipoCobrancaTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
