      ...
      <div class="content">
        <section>
          <div class="intro-services-to-be-billed">
            <h1 ref="title" class="title-page">Tipo de Histórico de Processamento</h1>
          </div>
          ...
          <div class="margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmTipoHistoricoProcessoTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="COD_PROCESSO"
  ...
  export default {
    ...
    methods: {
      ...
      rowClass(data) {
        return {
          'pointer-cursor': true,
          'active-row': this.selectedRow && this.selectedRow.COD_PROCESSO === data.COD_PROCESSO,
        };
      },
      async fetchData() {
        this.isLoading = true;
        try {
          ...
          const response = await axios.get(`${baseUrl}/TIPO_HISTORICO_PROCESSO_ALL`);
          this.frmTipoHistoricoProcessoTable = response.data; 
        } catch (error) {
      ...
      printPDF() {
        ...
        doc.autoTable({
          ...
          body: this.frmTipoHistoricoProcessoTable.map(row => this.columns.map(col => row[col.field])),
          theme: 'grid',
