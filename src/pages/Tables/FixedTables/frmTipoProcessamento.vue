      ...
      <div class="content">
        <section>
          <div class="intro-services-to-be-billed">
            <h1 ref="title" class="title-page">Tipo de Processamento</h1>
          </div>
          ...
          <div class="margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmTipoProcessamentoTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="TP_CODIGO"
    ...
    methods: {
      ...
      async fetchData() {
        ...
        try {
          ...
          const response = await axios.get(`${baseUrl}/TIPO_PROCESSAMENTO_ALL`);
          this.frmTipoProcessamentoTable = response.data; 
        } catch (error) {
      ...
      printPDF() {
        ...
        doc.autoTable({
          ...
          body: this.frmTipoProcessamentoTable.map(row => this.columns.map(col => row[col.field])),
          theme: 'grid',
