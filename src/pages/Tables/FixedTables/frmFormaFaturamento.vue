    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Forma de Faturamento</h1>
        </div>
        ...
        <div class="margin-top-30">
          ...
          <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
            :value="frmFormaFaturamentoTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="FF_CODIGO"
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {
          name: "FF_DESC_RESUMIDO",
          required: true,
          label: "Descrição Resumida",
          align: "left",
          field: "FF_DESC_RESUMIDO",
        },
        {
          name: "FF_IND_SERVICO",
          required: true,
          label: "Indicador de Serviço",
          align: "left",
          field: "FF_IND_SERVICO",
        },
        {
          name: "FF_IND_PROG_AGENTE",
          required: true,
          label: "Indicador de Programação Agente",
          align: "left",
          field: "FF_IND_PROG_AGENTE",
        }
      ],
      frmFormaFaturamentoTable: [],
      isLoading: false,
      error: null,
    };
  },
  methods: {
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.FF_CODIGO === data.FF_CODIGO,
      };
    },
    async fetchData() {
      this.isLoading = true;
      try {
        ...
        const response = await axios.get(`${baseUrl}/FORMA_FATURAMENTO_ALL`);
        this.frmFormaFaturamentoTable = response.data; 
      } catch (error) {
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmFormaFaturamentoTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
