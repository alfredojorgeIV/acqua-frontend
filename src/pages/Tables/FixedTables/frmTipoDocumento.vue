    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Tipo de Documento</h1>
        </div>
        ...
        <div class="margin-top-30">
          ...
          <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
            :value="frmTipoDocumentoTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="TIPO_DOCUMENTO"
...
export default {
  ...
  methods: {
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.TIPO_DOCUMENTO === data.TIPO_DOCUMENTO,
      };
    },
    async fetchData() {
      this.isLoading = true;
      try {
        ...
        const response = await axios.get(`${baseUrl}/TIPO_DOCUMENTO_ALL`);
        this.frmTipoDocumentoTable = response.data; 
      } catch (error) {
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmTipoDocumentoTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
