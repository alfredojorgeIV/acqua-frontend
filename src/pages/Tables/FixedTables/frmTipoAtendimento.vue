    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Tipo de Atendimento</h1>
        </div>
        ...
        <div class="margin-top-30">
          ...
          <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
            :value="frmTipoAtendimentoTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="TA_CODIGO"
  ...
  methods: {
    ...
    async fetchData() {
      ...
      try {
        ...
        this.frmTipoAtendimentoTable = response.data; 
      } catch (error) {
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmTipoAtendimentoTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
