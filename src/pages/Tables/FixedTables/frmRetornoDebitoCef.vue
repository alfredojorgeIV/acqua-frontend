      ...
      <div class="content">
        <section>
          <div class="intro-services-to-be-billed">
            <h1 ref="title" class="title-page">Retorno de Débito CEF</h1>
          </div>
          ...
          <div class="margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmRetornoDebitoCefTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="COD_RETORNO"
    ...
    methods: {
      ...
      async fetchData() {
        ...
        try {
          ...
          const response = await axios.get(`${baseUrl}/RETORNO_DEBITO_CEF_ALL`);
          this.frmRetornoDebitoCefTable = response.data; 
        } catch (error) {
      ...
      printPDF() {
        ...
        doc.autoTable({
          ...
          body: this.frmRetornoDebitoCefTable.map(row => this.columns.map(col => row[col.field])),
          theme: 'grid',
