      ...
      <div class="content">
        <section>
          <div class="intro-services-to-be-billed">
            <h1 ref="title" class="title-page">Situação do Orçamento</h1>
          </div>
          ...
          <div class="margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmSituacaoOrcamentoTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="SITUACAO_ORCAMENTO"
  ...
  export default {
    ...
    data() {
      return {
        ...
        columns: [
          {
            name: "SITUACAO_ORCAMENTO",
            required: true,
            label: "Código",
            align: "left",
            field: "SITUACAO_ORCAMENTO",
          },
          {
            name: "DESCR_TIPO",
            required: true,
    ...
    methods: {
      ...
      rowClass(data) {
        return {
          'pointer-cursor': true,
          'active-row': this.selectedRow && this.selectedRow.SITUACAO_ORCAMENTO === data.SITUACAO_ORCAMENTO,
        };
      },
      async fetchData() {
        this.isLoading = true;
        try {
          ...
          const response = await axios.get(`${baseUrl}/SITUACAO_ORCAMENTO_ALL`);
          this.frmSituacaoOrcamento = response.data; 
        } catch (error) {
      ...
      printPDF() {
        ...
        doc.autoTable({
          ...
          body: this.frmSituacaoOrcamentoTable.map(row => this.columns.map(col => row[col.field])),
          theme: 'grid',
