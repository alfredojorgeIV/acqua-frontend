    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Tipo de Retorno dos Correios</h1>
        </div>
        ...
        <div class="margin-top-30">
          ...
          <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
            :value="frmTipoRetCorreiosTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="TR_CODIGO"
  ...
  methods: {
    ...
    async fetchData() {
      ...
      try {
        ...
        const response = await axios.get(`${baseUrl}/TIPO_RETORNO_CORREIOS_ALL`);
        this.frmTipoRetCorreiosTable = response.data; 
      } catch (error) {
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmTipoRetCorreiosTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
