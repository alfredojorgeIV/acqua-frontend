    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Entrega de Conta</h1>
        </div>
        ...
        <div class="margin-top-30">
          ...
          <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
            :value="frmEntregaTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="TIPO_ENTREGA"
  ...
  methods: {
    ...
    async fetchData() {
      ...
      try {
        ...
        const response = await axios.get(`${baseUrl}/TIPO_ENTREGA_CONTA_ALL`);
        this.frmEntregaTable = response.data; 
      } catch (error) {
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmEntregaTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
