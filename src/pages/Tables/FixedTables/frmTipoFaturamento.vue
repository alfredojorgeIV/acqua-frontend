    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Tipo de Faturamento</h1>
        </div>
        ...
        <div class="margin-top-30">
          ...
          <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
            :value="frmTipoFaturamentoTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="TF_CODIGO"
  ...
  methods: {
    ...
    async fetchData() {
      ...
      try {
        ...
        const response = await axios.get(`${baseUrl}/TIPO_FATURAMENTO_ALL`);
        this.frmTipoFaturamentoTable = response.data; 
      } catch (error) {
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmTipoFaturamentoTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
