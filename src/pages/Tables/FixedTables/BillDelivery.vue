    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 class="title-page">Entrega de conta</h1>
        </div>
        ...
        <div class="margin-top-30">
          ...
          <DataTable style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
            :value="billDeliveryTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="id"
...
export default {
  ...
  methods: {
    async fetchData() {
      this.isLoading = true;
      try {
        const response = await axios.get('http://localhost:3000/TIPO_ENTREGA_CONTA_ALL');
        this.billDeliveryTable = response.data; 
      } catch (error) {
