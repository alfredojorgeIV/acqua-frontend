      ...
      <div class="content">
        <section>
          <div class="intro-services-to-be-billed">
            <h1 ref="title" class="title-page">Tipo de Unidade Estimada</h1>
          </div>
          ...
          <div class="margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmTipoUnidadeEstTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="TP_UNIDADE_EST"
  ...
  export default {
    ...
    methods: {
      ...
      rowClass(data) {
        return {
          'pointer-cursor': true,
          'active-row': this.selectedRow && this.selectedRow.TP_UNIDADE_EST === data.TP_UNIDADE_EST,
        };
      },
      async fetchData() {
        this.isLoading = true;
        try {
          ...
          const response = await axios.get(`${baseUrl}/TIPO_UNIDADE_EST_ALL`);
          this.frmTipoUnidadeEstTable = response.data; 
        } catch (error) {
      ...
      printPDF() {
        ...
        doc.autoTable({
          ...
          body: this.frmTipoUnidadeEstTable.map(row => this.columns.map(col => row[col.field])),
          theme: 'grid',
