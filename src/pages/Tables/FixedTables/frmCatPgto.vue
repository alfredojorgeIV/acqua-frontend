    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Categoria de Pagamento</h1>
        </div>
        ...
        <div class="margin-top-30">
          ...
          <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
            :value="frmCatPgtoTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="CATEG_PGTO"
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {
          name: "DESC_CATEG_PGTO",
          required: true,
          label: "Descrição",
          align: "left",
          field: "DESC_CATEG_PGTO",
        },
      ],
      frmCatPgtoTable: [],
      isLoading: false,
  ...
  methods: {
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.CATEG_PGTO === data.CATEG_PGTO,
      };
    },
    async fetchData() {
      this.isLoading = true;
      try {
        ...
        const response = await axios.get(`${baseUrl}/CATEGORIA_PGTO_ALL`);
        this.frmCatPgtoTable = response.data; 
      } catch (error) {
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmCatPgtoTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
