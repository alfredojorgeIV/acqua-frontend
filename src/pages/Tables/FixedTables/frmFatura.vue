    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Tipo de Fatura</h1>
        </div>
        ...
        <div class="margin-top-30">
          ...
          <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
            :value="frmFaturaTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="TIPO_FATURA"
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {
          name: "DESC_TIPO_FATURA",
          required: true,
          label: "Descrição",
          align: "left",
          field: "DESC_TIPO_FATURA",
        },
      ],
      frmFaturaTable: [],
      isLoading: false,
  ...
  methods: {
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.TIPO_FATURA === data.TIPO_FATURA,
      };
    },
    async fetchData() {
      this.isLoading = true;
      try {
        ...
        const response = await axios.get(`${baseUrl}/TIPO_FATURA_ALL`);
        this.frmFaturaTable = response.data; 
      } catch (error) {
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmFaturaTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
