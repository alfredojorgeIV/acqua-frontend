    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Grupo de Serviço</h1>
        </div>
        ...
        <div class="margin-top-30">
          ...
          <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
            :value="frmGrupoServicoTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="GRUPO_SERVICO"
...
export default {
  ...
  methods: {
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.GRUPO_SERVICO === data.GRUPO_SERVICO,
      };
    },
    async fetchData() {
      this.isLoading = true;
      try {
        ...
        const response = await axios.get(`${baseUrl}/GRUPO_SERVICO_ALL`);
        this.frmGrupoServicoTable = response.data; 
      } catch (error) {
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmGrupoServicoTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
