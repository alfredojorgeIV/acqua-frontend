    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 class="title-page">Tipo de vias</h1>
        </div>
        ...
        <div class="margin-top-30">
          ...
          <DataTable style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
            :value="copysTypeTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="id"
...
export default {
  ...
  methods: {
    async fetchData() {
      this.isLoading = true;
      try {
        const response = await axios.get('http://localhost:3000/VIAS_ALL');
        this.copysTypeTable = response.data;
      } catch (error) {
