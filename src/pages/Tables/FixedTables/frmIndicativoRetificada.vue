      ...
      <div class="content">
        <section>
          <div class="intro-services-to-be-billed">
            <h1 ref="title" class="title-page">Indicativo Retificada</h1>
          </div>
          ...
          <div class="margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmIndicativoRetificadaTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="IR_DESCRICAO"
  ...
  export default {
    ...
    methods: {
      ...
      rowClass(data) {
        return {
          'pointer-cursor': true,
          'active-row': this.selectedRow && this.selectedRow.IR_CODIGO === data.IR_CODIGO,
        };
      },
      async fetchData() {
        this.isLoading = true;
        try {
          ...
          const response = await axios.get(`${baseUrl}/INDICATIVO_RETIFICADA_ALL`);
          this.frmIndicativoRetificadaTable = response.data; 
        } catch (error) {
      ...
      printPDF() {
        ...
        doc.autoTable({
          ...
          body: this.frmIndicativoRetificadaTable.map(row => this.columns.map(col => row[col.field])),
          theme: 'grid',
