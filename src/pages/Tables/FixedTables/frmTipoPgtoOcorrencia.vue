      ...
      <div class="content">
        <section>
          <div class="intro-services-to-be-billed">
            <h1 ref="title" class="title-page">Tipo de Ocorrência de Pagamento</h1>
          </div>
          ...
          <div class="margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmTipoPgtoOcorrenciaTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="TIPO_PGTO_OCORRENCIA"
  ...
  export default {
    ...
    data() {
      return {
        ...
        columns: [
          {
            name: "TIPO_PGTO_OCORRENCIA",
            required: true,
            label: "Código",
            align: "left",
            field: "TIPO_PGTO_OCORRENCIA",
          },
          {
            name: "DESC_PGTO_OCORRENCIA",
            required: true,
            label: "Descrição",
            align: "left",
            field: "DESC_PGTO_OCORRENCIA",
          },
        ],
        frmTipoPgtoOcorrenciaTable: [],
        isLoading: false,
        error: null,
      };
    },
    methods: {
      ...
      rowClass(data) {
        return {
          'pointer-cursor': true,
          'active-row': this.selectedRow && this.selectedRow.TIPO_PGTO_OCORRENCIA === data.TIPO_PGTO_OCORRENCIA,
        };
      },
      async fetchData() {
        this.isLoading = true;
        try {
          ...
          const response = await axios.get(`${baseUrl}/TIPO_PGTO_OCORRENCIA_ALL`);
          this.frmTipoPgtoOcorrenciaTable = response.data; 
        } catch (error) {
      ...
      printPDF() {
        ...
        doc.autoTable({
          ...
          body: this.frmTipoPgtoOcorrenciaTable.map(row => this.columns.map(col => row[col.field])),
          theme: 'grid',
