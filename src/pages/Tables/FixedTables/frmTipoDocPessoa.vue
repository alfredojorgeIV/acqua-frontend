      ...
      <div class="content">
        <section>
          <div class="intro-services-to-be-billed">
            <h1 ref="title" class="title-page">Tipo de Documento Pessoal</h1>
          </div>
          ...
          <div class="margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmTipoDocPessoaTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="TIPO_DOC_PESSOA"
  ...
  export default {
        ...
        columns: [
          {
            ...
            label: "Código",
            align: "left",
            field: "TIPO_DOC_PESSOA",
          },
          {
            name: "DESC_DOC_PESSOA",
            required: true,
            label: "Descrição",
            align: "left",
            field: "DESC_DOC_PESSOA",
          },
        ],
        frmTipoDocPessoaTable: [],
        isLoading: false,
        error: null,
      };
    },
    methods: {
      ...
      rowClass(data) {
        return {
          'pointer-cursor': true,
          'active-row': this.selectedRow && this.selectedRow.TIPO_DOC_PESSOA === data.TIPO_DOC_PESSOA,
        };
      },
      async fetchData() {
        this.isLoading = true;
        try {
          ...
          const response = await axios.get(`${baseUrl}/TIPO_DOC_PESSOA_ALL`);
          this.frmTipoDocPessoaTable = response.data; 
        } catch (error) {
      ...
      printPDF() {
        ...
        doc.autoTable({
          ...
          body: this.frmTipoDocPessoaTable.map(row => this.columns.map(col => row[col.field])),
          theme: 'grid',
