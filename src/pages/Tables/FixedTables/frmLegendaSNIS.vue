      ...
      <div class="content">
        <section>
          <div class="intro-services-to-be-billed">
            <h1 ref="title" class="title-page">Legenda SNIS</h1>
          </div>
          ...
          <div class="margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmLegendaSNISTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="CODIGO"
    ...
    methods: {
      ...
      async fetchData() {
        ...
        try {
          ...
          const response = await axios.get(`${baseUrl}/LEGENDA_SNIS_ALL`);
          this.frmLegendaSNISTable = response.data; 
        } catch (error) {
      ...
      printPDF() {
        ...
        doc.autoTable({
          ...
          body: this.frmLegendaSNISTable.map(row => this.columns.map(col => row[col.field])),
          theme: 'grid',
