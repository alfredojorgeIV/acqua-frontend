      ...
      <div class="content">
        <section>
          <div class="intro-services-to-be-billed">
            <h1 ref="title" class="title-page">Situação da OS</h1>
          </div>
          ...
          <div class="margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmSituacaoOSTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="SITUACAO_OS"
  ...
  export default {
    ...
    methods: {
      ...
      rowClass(data) {
        return {
          'pointer-cursor': true,
          'active-row': this.selectedRow && this.selectedRow.SITUACAO_OS === data.SITUACAO_OS,
        };
      },
      async fetchData() {
        this.isLoading = true;
        try {
          ...
          const response = await axios.get(`${baseUrl}/SITUACAO_OS_ALL`);
          this.frmSituacaoOSTable = response.data; 
        } catch (error) {
      ...
      printPDF() {
        ...
        doc.autoTable({
          ...
          body: this.frmSituacaoOSTable.map(row => this.columns.map(col => row[col.field])),
          theme: 'grid',
