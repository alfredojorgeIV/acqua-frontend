      ...
      <div class="content">
        <section>
          <div class="intro-services-to-be-billed">
            <h1 ref="title" class="title-page">Tipo de Rateio de Consumo</h1>
          </div>
          ...
          <div class="margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmTipoRateioConsumoTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="COD_RATEIO"
    ...
    methods: {
      ...
      async fetchData() {
        ...
        try {
          ...
          const response = await axios.get(`${baseUrl}/TIPO_RATEIO_CONSUMO_ALL`);
          this.frmTipoRateioConsumoTable = response.data; 
        } catch (error) {
      ...
      printPDF() {
        ...
        doc.autoTable({
          ...
          body: this.frmTipoRateioConsumoTable.map(row => this.columns.map(col => row[col.field])),
          theme: 'grid',
