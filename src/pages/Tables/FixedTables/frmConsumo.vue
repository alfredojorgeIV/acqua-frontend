    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Situação do Consumo</h1>
        </div>
        ...
        <div class="margin-top-30">
          ...
          <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
            :value="frmConsumoTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="SIT_CONSUMO"
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {
          name: "DESC_SIT_CONSUMO",
          required: true,
          label: "Descrição",
          align: "left",
          field: "DESC_SIT_CONSUMO",
        },
      ],
      frmConsumoTable: [],
      isLoading: false,
  ...
  methods: {
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.SIT_CONSUMO === data.SIT_CONSUMO,
      };
    },
    async fetchData() {
      this.isLoading = true;
      try {
        ...
        const response = await axios.get(`${baseUrl}/SITUACAO_CONSUMO_ALL`);
        this.frmConsumoTable = response.data; 
      } catch (error) {
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmConsumoTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
