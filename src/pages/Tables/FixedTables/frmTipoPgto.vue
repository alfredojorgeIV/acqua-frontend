    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Tipo de Pagamento</h1>
        </div>
        ...
        <div class="margin-top-30">
          ...
          <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
            :value="frmTipoPgtoTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="TIPO_PAGAMENTO"
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {
          name: "DESC_TP_PAGAMENTO",
          required: true,
          label: "Descrição",
          align: "left",
          field: "DESC_TP_PAGAMENTO",
        },
      ],
      frmTipoPgtoTable: [],
      isLoading: false,
  ...
  methods: {
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.TIPO_PAGAMENTO === data.TIPO_PAGAMENTO,
      };
    },
    async fetchData() {
      this.isLoading = true;
      try {
        ...
        const response = await axios.get(`${baseUrl}/TIPO_PAGAMENTO_ALL`);
        this.frmTipoPgtoTable = response.data; 
      } catch (error) {
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmTipoPgtoTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
