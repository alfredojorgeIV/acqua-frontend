      ...
      <div class="content">
        <section>
          <div class="intro-services-to-be-billed">
            <h1 ref="title" class="title-page">Tipo de Cobrança Mínima</h1>
          </div>
          ...
          <div class="margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmTipoCobrancaMinimaTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="CODIGO_COBRANCA"
  ...
  export default {
    ...
    methods: {
      ...
      rowClass(data) {
        return {
          'pointer-cursor': true,
          'active-row': this.selectedRow && this.selectedRow.CODIGO_COBRANCA === data.CODIGO_COBRANCA,
        };
      },
      async fetchData() {
        this.isLoading = true;
        try {
          ...
          const response = await axios.get(`${baseUrl}/TIPO_COBRANCA_MINIMA_ALL`);
          this.frmTipoCobrancaMinimaTable = response.data; 
        } catch (error) {
      ...
      printPDF() {
        ...
        doc.autoTable({
          ...
          body: this.frmTipoCobrancaMinimaTable.map(row => this.columns.map(col => row[col.field])),
          theme: 'grid',
