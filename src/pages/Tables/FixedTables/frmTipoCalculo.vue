      ...
      <div class="content">
        <section>
          <div class="intro-services-to-be-billed">
            <h1 ref="title" class="title-page">Tipo de Cálculo</h1>
          </div>
          ...
          <div class="margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmTipoCalculoTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="TC_CODIGO"
    ...
    data() {
      return {
        ...
        columns: [
          ...
          {
            name: "TC_IND_DIAS_CONSUMO",
            required: true,
            label: "Dias",
            align: "left",
            field: "TC_IND_DIAS_CONSUMO",
          },
          {
            name: "TC_MIN_DIAS_CONSUMO",
            required: true,
            label: "Mín. dias de consumo",
            align: "left",
            field: "TC_MIN_DIAS_CONSUMO",
          },
          {
            name: "TC_VARIA_DIAS_MAX",
            required: true,
            label: "Max. de variação de dias",
            align: "left",
            field: "TC_VARIA_DIAS_MAX",
          },
          {
            name: "TC_IND_ESCALONADO",
            required: true,
            label: "Escalonado",
            align: "left",
            field: "TC_IND_ESCALONADO",
          },
          {
            name: "TC_IND_FIXO",
            required: true,
    ...
    methods: {
      ...
      async fetchData() {
        ...
        try {
          ...
          const response = await axios.get(`${baseUrl}/TIPO_CALCULO_ALL`);
          this.frmTipoCalculoTable = response.data; 
        } catch (error) {
      ...
      printPDF() {
        ...
        doc.autoTable({
          ...
          body: this.frmTipoCalculoTable.map(row => this.columns.map(col => row[col.field])),
          theme: 'grid',
