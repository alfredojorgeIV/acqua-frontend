    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Motivo de Não Faturamento</h1>
        </div>
        ...
        <div class="margin-top-30">
          ...
          <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
            :value="frmMotivoNaoFatTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="MF_MOTIVO"
...
export default {
  ...
  methods: {
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.MF_MOTIVO === data.MF_MOTIVO,
      };
    },
    async fetchData() {
      this.isLoading = true;
      try {
        ...
        const response = await axios.get(`${baseUrl}/MOTIVO_NAO_FATURAMENTO_ALL`);
        this.frmMotivoNaoFatTable = response.data; 
      } catch (error) {
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmMotivoNaoFatTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
