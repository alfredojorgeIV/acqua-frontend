      ...
      <div class="content">
        <section>
          <div class="intro-services-to-be-billed">
            <h1 ref="title" class="title-page">Tipo Data Grupo Leitura</h1>
          </div>
          ...
          <div class="margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmTipoDataGrupoLeituraTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="TD_CODIGO"
  ...
  export default {
    ...
    methods: {
      ...
      rowClass(data) {
        return {
          'pointer-cursor': true,
          'active-row': this.selectedRow && this.selectedRow.TD_CODIGO === data.TD_CODIGO,
        };
      },
      async fetchData() {
        this.isLoading = true;
        try {
          ...
          const response = await axios.get(`${baseUrl}/TIPO_DATA_GRUPO_LEITURA_ALL`);
          this.frmTipoDataGrupoLeituraTable = response.data; 
        } catch (error) {
      ...
      printPDF() {
        ...
        doc.autoTable({
          ...
          body: this.frmTipoDataGrupoLeituraTable.map(row => this.columns.map(col => row[col.field])),
          theme: 'grid',
