    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Grupo de Ocorrências</h1>
        </div>
        ...
        <div class="margin-top-30">
          ...
          <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
            :value="frmGrpAnormTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="GRUPO_ANORM"
...
export default {
  ...
  methods: {
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.GRUPO_ANORM === data.GRUPO_ANORM,
      };
    },
    async fetchData() {
      this.isLoading = true;
      try {
        ...
        const response = await axios.get(`${baseUrl}/GRUPO_ANORMALIDADE_ALL`);
        this.frmGrpAnormTable = response.data; 
      } catch (error) {
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmGrpAnormTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
