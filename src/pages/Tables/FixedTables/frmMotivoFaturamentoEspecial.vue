      ...
      <div class="content">
        <section>
          <div class="intro-services-to-be-billed">
            <h1 ref="title" class="title-page">Motivo de Faturamento Especial</h1>
          </div>
          ...
          <div class="margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmMotivoFaturamentoEspecialTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="MF_CODIGO"
  ...
  export default {
    ...
    data() {
      return {
        ...
        frmMotivoFaturamentoEspecialTable: [],
        isLoading: false,
        error: null,
      };
    },
    methods: {
      ...
      rowClass(data) {
        return {
          'pointer-cursor': true,
          'active-row': this.selectedRow && this.selectedRow.MF_CODIGO === data.MF_CODIGO,
        };
      },
      async fetchData() {
        this.isLoading = true;
        try {
          ...
          const response = await axios.get(`${baseUrl}/MOTIVO_FATURAMENTO_ESPECIAL_ALL`);
          this.frmMotivoFaturamentoEspecialTable = response.data; 
        } catch (error) {
      ...
      printPDF() {
        ...
        doc.autoTable({
          ...
          body: this.frmMotivoFaturamentoEspecialTable.map(row => this.columns.map(col => row[col.field])),
          theme: 'grid',
