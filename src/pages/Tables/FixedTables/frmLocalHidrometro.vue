      ...
      <div class="content">
        <section>
          <div class="intro-services-to-be-billed">
            <h1 ref="title" class="title-page">Local do HD</h1>
          </div>
          ...
          <div class="margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmLocalHidrometroTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="LH_CODIGO"
  ...
  export default {
    ...
    methods: {
      ...
      rowClass(data) {
        return {
          'pointer-cursor': true,
          'active-row': this.selectedRow && this.selectedRow.LH_CODIGO === data.LH_CODIGO,
        };
      },
      async fetchData() {
        this.isLoading = true;
        try {
          ...
          const response = await axios.get(`${baseUrl}/LOCAL_HIDROMETRO_ALL`);
          this.frmLocalHidrometroTable = response.data; 
        } catch (error) {
      ...
      printPDF() {
        ...
        doc.autoTable({
          ...
          body: this.frmLocalHidrometroTable.map(row => this.columns.map(col => row[col.field])),
          theme: 'grid',
