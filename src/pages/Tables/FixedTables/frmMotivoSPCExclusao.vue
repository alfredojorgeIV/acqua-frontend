      ...
      <div class="content">
        <section>
          <div class="intro-services-to-be-billed">
            <h1 ref="title" class="title-page">Motivo de Exclusão do SPC</h1>
          </div>
          ...
          <div class="margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmMotivoSPCExclusaoTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="CODIGO_MOTIVO"
  ...
  export default {
    ...
    methods: {
      ...
      rowClass(data) {
        return {
          'pointer-cursor': true,
          'active-row': this.selectedRow && this.selectedRow.CODIGO_MOTIVO === data.CODIGO_MOTIVO,
        };
      },
      async fetchData() {
        this.isLoading = true;
        try {
          ...
          const response = await axios.get(`${baseUrl}/MOTIVO_SPC_EXCLUSAO_ALL`);
          this.frmMotivoSPCExclusaoTable = response.data; 
        } catch (error) {
      ...
      printPDF() {
        ...
        doc.autoTable({
          ...
          body: this.frmMotivoSPCExclusaoTable.map(row => this.columns.map(col => row[col.field])),
          theme: 'grid',
