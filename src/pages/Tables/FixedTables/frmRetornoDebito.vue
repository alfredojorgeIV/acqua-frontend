    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Retorno de Débito</h1>
        </div>
        ...
        <div class="margin-top-30">
          ...
          <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
            :value="frmRetornoDebitoTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="COD_RETORNO"
...
export default {
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          name: "IND_DEBITO",
          required: true,
          label: "Indicativo de débito",
          align: "left",
          field: "IND_DEBITO",
        },
        {
          name: "COD_RETORNO_ALF",
          required: true,
          label: "Código de retorno Alfa",
          align: "left",
          field: "COD_RETORNO_ALF",
        }
        
      ],
      frmRetornoDebitoTable: [],
      isLoading: false,
      error: null,
    };
  },
  methods: {
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.COD_RETORNO === data.COD_RETORNO,
      };
    },
    async fetchData() {
      this.isLoading = true;
      try {
        ...
        const response = await axios.get(`${baseUrl}/RETORNO_DEBITO_ALL`);
        this.frmRetornoDebitoTable = response.data; 
      } catch (error) {
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmRetornoDebitoTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
