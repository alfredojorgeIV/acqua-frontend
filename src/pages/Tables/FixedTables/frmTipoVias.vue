    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Tipo de Vias</h1>
        </div>
        ...
        <div class="margin-top-30">
          ...
          <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
            :value="frmTipoViasTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="VI_MOTIVO"
...
export default {
  ...
  methods: {
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.VI_MOTIVO === data.VI_MOTIVO,
      };
    },
    async fetchData() {
      this.isLoading = true;
      try {
        ...
        const response = await axios.get(`${baseUrl}/VIAS_ALL`);
        this.frmTipoViasTable = response.data; 
      } catch (error) {
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmTipoViasTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
