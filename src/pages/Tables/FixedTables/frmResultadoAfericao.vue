      ...
      <div class="content">
        <section>
          <div class="intro-services-to-be-billed">
            <h1 ref="title" class="title-page">Resultado de Afreição do HD</h1>
          </div>
          ...
          <div class="margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmResultadoAfericaoTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="RA_CODIGO"
  ...
  export default {
    ...
    methods: {
      ...
      rowClass(data) {
        return {
          'pointer-cursor': true,
          'active-row': this.selectedRow && this.selectedRow.RA_CODIGO === data.RA_CODIGO,
        };
      },
      async fetchData() {
        this.isLoading = true;
        try {
          ...
          const response = await axios.get(`${baseUrl}/RESULTADO_AFERICAO_ALL`);
          this.frmResultadoAfericaoTable = response.data; 
        } catch (error) {
      ...
      printPDF() {
        ...
        doc.autoTable({
          ...
          body: this.frmResultadoAfericaoTable.map(row => this.columns.map(col => row[col.field])),
          theme: 'grid',
