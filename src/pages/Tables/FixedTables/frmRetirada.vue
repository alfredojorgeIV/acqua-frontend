    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Motivo de Retirada do HD</h1>
        </div>
        ...
        <div class="margin-top-30">
          ...
          <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
            :value="frmRetiradaTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="MOTIVO_RETIR_HD"
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        {
          name: "MOTIVO_RETIR_HD",
          required: true,
          label: "Código",
          align: "left",
          field: "MOTIVO_RETIR_HD",
        },
        {
          name: "DESC_MTV_RETIR_HD",
          required: true,
          label: "Descrição",
          align: "left",
          field: "DESC_MTV_RETIR_HD",
        },
      ],
      frmRetiradaTable: [],
      isLoading: false,
  ...
  methods: {
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.MOTIVO_RETIR_HD === data.MOTIVO_RETIR_HD,
      };
    },
    async fetchData() {
      this.isLoading = true;
      try {
        ...
        const response = await axios.get(`${baseUrl}/MOTIVO_RETIR_HD_ALL`);
        this.frmRetiradaTable = response.data; 
      } catch (error) {
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmRetiradaTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
