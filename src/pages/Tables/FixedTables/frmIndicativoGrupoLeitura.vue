<template>
      ...
      <div class="content">
        <section>
          <div class="intro-services-to-be-billed">
            <h1 ref="title" class="title-page">Indicativo de Grupo de Leitura</h1>
          </div>
          ...
          <div class="margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmIndicativoGrupoLeituraTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="IG_CODIGO"
  ...
  export default {
    name: "frmIndicativoConsumoLeitura",
    components: {
      MenuApp,
      CompanyTag,
      DataTable,
    ...
    data() {
      return {
        ...
        frmIndicativoGrupoLeituraTable: [],
        isLoading: false,
        error: null,
      };
    },
    methods: {
      ...
      rowClass(data) {
        return {
          'pointer-cursor': true,
          'active-row': this.selectedRow && this.selectedRow.IG_CODIGO === data.IG_CODIGO,
        };
      },
      async fetchData() {
        this.isLoading = true;
        try {
          ...
          const response = await axios.get(`${baseUrl}/INDICATIVO_GRUPO_LEITURA_ALL`);
          this.frmIndicativoGrupoLeituraTable = response.data; 
        } catch (error) {
      ...
      printPDF() {
        ...
        doc.autoTable({
          ...
          body: this.frmIndicativoGrupoLeituraTable.map(row => this.columns.map(col => row[col.field])),
          theme: 'grid',
