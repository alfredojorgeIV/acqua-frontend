      ...
      <div class="content">
        <section>
          <div class="intro-services-to-be-billed">
            <h1 ref="title" class="title-page">Nível de Contaminação</h1>
          </div>
          ...
          <div class="margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmRilesTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="NC_CODIGO"
  ...
  export default {
    ...
    methods: {
      ...
      rowClass(data) {
        return {
          'pointer-cursor': true,
          'active-row': this.selectedRow && this.selectedRow.NC_CODIGO === data.NC_CODIGO,
        };
      },
      async fetchData() {
        this.isLoading = true;
        try {
          ...
          const response = await axios.get(`${baseUrl}/NIVEL_CONTAMINACAO_ALL`);
          this.frmRilesTable = response.data; 
        } catch (error) {
      ...
      printPDF() {
        ...
        doc.autoTable({
          ...
          body: this.frmRilesTable.map(row => this.columns.map(col => row[col.field])),
          theme: 'grid',
