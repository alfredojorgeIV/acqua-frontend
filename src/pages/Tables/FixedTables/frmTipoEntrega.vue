      ...
      <div class="content">
        <section>
          <div class="intro-services-to-be-billed">
            <h1 ref="title" class="title-page">Tipo de Entrega de Conta</h1>
          </div>
          ...
          <div class="margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmTipoEntregaTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="TIPO_ENTREGA"
  ...
  export default {
    ...
    methods: {
      ...
      rowClass(data) {
        return {
          'pointer-cursor': true,
          'active-row': this.selectedRow && this.selectedRow.TIPO_ENTREGA === data.TIPO_ENTREGA,
        };
      },
      async fetchData() {
        this.isLoading = true;
        try {
          ...
          const response = await axios.get(`${baseUrl}/TIPO_ENTREGA_CLI_ALL`);
          this.frmTipoEntregaTable = response.data; 
        } catch (error) {
      ...
      printPDF() {
        ...
        doc.autoTable({
          ...
          body: this.frmTipoEntregaTable.map(row => this.columns.map(col => row[col.field])),
          theme: 'grid',
