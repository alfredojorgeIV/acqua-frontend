    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Situação da Fatura</h1>
        </div>
        ...
        <div class="margin-top-30">
          ...
          <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
            :value="frmPagamentoTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="SIT_PAGTO"
...
export default {
  ...
  methods: {
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.SIT_PAGTO === data.SIT_PAGTO,
      };
    },
    async fetchData() {
      this.isLoading = true;
      try {
        ...
        const response = await axios.get(`${baseUrl}/SITUACAO_PAGAMENTO_ALL`);
        this.frmPagamentoTable = response.data; 
      } catch (error) {
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmPagamentoTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
