<template>
      ...
      <div class="content">
        <section>
          <div class="intro-services-to-be-billed">
            <h1 class="title-page">Tipo de vazamento</h1>
          </div>
          ...
          <DataTable style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
            :value="frmVazamentoTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10"
            dataKey="TV_CODIGO" @row-click="handleRowClick" :rowClass="rowClass">
  ...
  export default {
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.TV_CODIGO === data.TV_CODIGO,
      };
    },
      async fetchData() {
        this.isLoading = true;
        try {
          const response = await axios.get('http://localhost:3000/TIPO_VAZAMENTO_ALL');
          this.frmVazamentoTable = response.data;
        } catch (error) {
