    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 class="title-page">Tipo de consumidor</h1>
        </div>
        <div class="flex gap-4">
          <div class="w-61 margin-top-30">
            ...
            <DataTable style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="consumerTypeTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="id"
          ...
          <div class="w-35">
            <div class="box-tables-info" v-if="selectedRow && !showAddBox">
              ...
              <input v-model="selectedRow.TIPO_CONSUMIDOR" class="inputs margin-bottom-30" :disabled="!isEditing" />
              <p class="p-labels">Descrição</p>
              <input v-model="selectedRow.DESC_CONSUMIDOR" class="inputs margin-bottom-30" :disabled="!isEditing" />
            ...
            <div class="box-tables-info" v-if="showAddBox && !selectedRow">
              <p class="p-labels">Código</p>
              <input class="inputs margin-bottom-30" v-model="addTIPO_CONSUMIDOR" />
              <p class="p-labels">Descrição</p>
              <input class="inputs margin-bottom-30" v-model="addDescricao" />

              <div class="flex justify-center gap-4 mt-40">
                <Button class="primary-button flex align-center" @click="addTIPO_CONSUMIDORTarifa">
...
export default {
  ...
  data() {
    return {
      ...
      consumerTypeTable: [],
      addTIPO_CONSUMIDOR: '', // Corrected
      addDescricao: '', // Corrected
    };
  },
  ...
  methods: {
    handleAddRowClick(event) {
      ...
      this.addTIPO_CONSUMIDOR = ''; // Corrected
    ...
    async addTIPO_CONSUMIDORTarifa() {
      this.isLoading = true;
      try {
        const response = await axios.post('http://localhost:3000/TIPO_CONSUMIDOR_ADD', {
          TIPO_CONSUMIDOR: Number(this.addTIPO_CONSUMIDOR), // Corrected
          DESC_CONSUMIDOR: this.addDescricao,
          NOME_ATUALIZ: "Luiza"
        });
    ...
    async fetchData() {
      this.isLoading = true;
      try {
        const response = await axios.get('http://localhost:3000/TIPO_CONSUMIDOR_ALL');
        this.data = response.data;
        this.consumerTypeTable = this.data;
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
      } finally {
    ...
    async deleteTaxCategory() {
      this.isLoading = true;
      try {
        const response = await axios.delete(`http://localhost:3000/TIPO_CONSUMIDOR_DELETE/${this.selectedRow.TIPO_CONSUMIDOR}`);
        console.log(response.data);
        await this.fetchData();
    ...
    async saveUpdate() {
      this.isLoading = true;
      try {
        const response = await axios.post(`http://localhost:3000/TIPO_CONSUMIDOR_UPDATE`, {
          TIPO_CONSUMIDOR: this.selectedRow.TIPO_CONSUMIDOR,
          DESC_CONSUMIDOR: this.selectedRow.DESC_CONSUMIDOR,
          NOME_ATUALIZ: "Luiza"
        });

        if (response.status === 200) {
          this.consumerTypeTable = this.consumerTypeTable.map(row => row.TIPO_CONSUMIDOR === this.selectedRow.TIPO_CONSUMIDOR ? this.selectedRow : row);
          this.isEditing = false;
        } else {
          console.error("Erro ao atualizar a TIPO_CONSUMIDOR de tarifa");
        }
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
    ...
    cancelAdd() {
      this.showAddBox = false;
      this.addTIPO_CONSUMIDOR = ''; // Clear input
