    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Motivo do Corte</h1>
        </div>
        <div class="flex gap-4">
          <div class="w-61 margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmCorteTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10"
              dataKey="MOTIVO_CORTE" @row-click="handleRowClick" :rowClass="rowClass">
          ...
          <div class="w-35">
            <div class="box-tables-info" v-if="selectedRow && !showAddBox">
              ...
              <input v-model="selectedRow.MOTIVO_CORTE" maxlength="5" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
              <p class="p-labels">Descrição</p>
              <input v-model="selectedRow.DESC_MOTIVO_CORTE" maxlength="15" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
            ...
            <div class="box-tables-info" v-if="showAddBox && !selectedRow">
              <p class="p-labels">Código</p>
              <input class="inputs margin-bottom-30" maxlength="5" v-model="addMotivoCorte" />
              <p class="p-labels">Descrição</p>
              <input class="inputs margin-bottom-30" maxlength="15" v-model="addDescMotivoCorte" />
              <div class="flex justify-center gap-4 mt-40">
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {
          name: "DESC_MOTIVO_CORTE",
          required: true,
          label: "Descrição",
          align: "left",
          field: "DESC_MOTIVO_CORTE",
        },
      ],
      frmCorteTable: []
    };
  ...
  methods: {
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.MOTIVO_CORTE === data.MOTIVO_CORTE
      };
    },
    ...
    async addToTable() {
      ...
      try {
        const response = await axios.post(`${baseUrl}/MOTIVO_CORTE_ADD`, {
          MOTIVO_CORTE: Number(this.addMotivoCorte),
          DESC_MOTIVO_CORTE: this.addDescMotivoCorte,
    ...
    async fetchData() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.get(`${baseUrl}/MOTIVO_CORTE_ALL`);
        this.data = response.data;
        this.frmCorteTable = this.data;
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
      } finally {
    ...
    async deleteFromTable() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.delete(`${baseUrl}/MOTIVO_CORTE_DELETE/${this.selectedRow.MOTIVO_CORTE}/${user.username}`);
        console.log(response.data);
        await this.fetchData();
    ...
    async saveUpdate() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.post(`${baseUrl}/MOTIVO_CORTE_UPDATE`, {
          MOTIVO_CORTE: Number(this.selectedRow.MOTIVO_CORTE),
          DESC_MOTIVO_CORTE: this.selectedRow.DESC_MOTIVO_CORTE,
          NOME_ATUALIZ: user.username
        });

        if (response.status === 200) {
          this.frmCorteTable = this.frmCorteTable.map(row =>
            row.MOTIVO_CORTE === this.selectedRow.MOTIVO_CORTE ? this.selectedRow : row
          );
          this.isEditing = false;
        } else {
          console.error("Erro ao atualizar o Motivo do Corte");
        }
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmCorteTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
      });
