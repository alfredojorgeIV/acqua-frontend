    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 class="title-page">Motivo da Falta</h1>
        </div>
        <div class="flex gap-4">
          <div class="w-61 margin-top-30">
            ...
            <DataTable style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="absenceReasonTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="id"
          ...
          <div class="w-35">
            <div class="box-tables-info" v-if="selectedRow && !showAddBox">
              ...
              <input v-model="selectedRow.CATEGORIA" class="inputs margin-bottom-30" :disabled="!isEditing" />
              <p class="p-labels">Descrição</p>
              <input v-model="selectedRow.DESCRICAO" class="inputs margin-bottom-30" :disabled="!isEditing" />
            ...
            <div class="box-tables-info" v-if="showAddBox && !selectedRow">
              ...
              <div class="flex justify-center gap-4 mt-40">
                <Button class="primary-button flex align-center" @click="addMotivoFalta">
          ...
          <Dialog header="Confirmar exclusão" :visible="showDeleteConfirmation" @hide="showDeleteConfirmation = false">
            ...
            <div class="flex justify-center mt-30 gap-5">
              ...
              <Button class="danger-button" @click="deleteMotivoFalta">Excluir</Button>
            </div>
          </Dialog>
        </div>
      </section>
...
export default {
  ...
  methods: {
    ...
    async addMotivoFalta() {
      this.isLoading = true;
      try {
        const response = await axios.post('http://localhost:3000/MOTIVO_FALTA_ADD', {
    ...
    async fetchData() {
      this.isLoading = true;
      try {
        const response = await axios.get('http://localhost:3000/MOTIVO_FALTA_ALL');
        this.data = response.data;
        this.absenceReasonTable = this.data;
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
      } finally {
    ...
    async deleteMotivoFalta() {
      this.isLoading = true;
      try {
        const response = await axios.delete(`http://localhost:3000/MOTIVO_FALTA_DELETE/${this.selectedRow.CATEGORIA}`);
        console.log(response.data);
        await this.fetchData();
    ...
    async saveUpdate() {
      this.isLoading = true;
      try {
        const response = await axios.post(`http://localhost:3000/MOTIVO_FALTA_UPDATE`, {
        ...
        if (response.status === 200) {
          this.absenceReasonTable = this.absenceReasonTable.map(row => row.CATEGORIA === this.selectedRow.CATEGORIA ? this.selectedRow : row);
          this.isEditing = false;
        } else {
