        ...
        <div class="flex gap-4">
          <div class="w-61 margin-top-30">
            ...
            <DataTable style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmLocalTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="LOCAL"
          ...
          <div class="w-35">
            <div class="box-tables-info" v-if="selectedRow && !showAddBox">
              ...
              <input v-model="selectedRow.LOCAL" @input="validateNumber" class="inputs margin-bottom-30" :disabled="!isEditing" />
              <p class="p-labels">Nome</p>
              <input v-model="selectedRow.NOME_LOCAL" maxlength="30" class="inputs margin-bottom-30" :disabled="!isEditing" />
              <p class="p-labels">Nome abreviado</p>
              <input v-model="selectedRow.NOME_ABREVIADO" maxlength="20" class="inputs margin-bottom-30" :disabled="!isEditing" />
            ...
            <div class="box-tables-info" v-if="showAddBox && !selectedRow">
              ...
              <p class="p-labels">Nome</p>
              <input class="inputs margin-bottom-30" maxlength="30" v-model="addNomeLocal" />
              <p class="p-labels">Nome abreviado</p>
              <input class="inputs margin-bottom-30" maxlength="20" v-model="addNomeAbrev" />
              <div class="flex justify-center gap-4 mt-40">
...
export default {
  ...
  methods: {
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.LOCAL === data.LOCAL
      };
    },
    ...
    async addToTable() {
      this.isLoading = true;
      try {
        const response = await axios.post('http://localhost:3000/LOCAL_ADD', {
          LOCAL: Number(this.addLocal),
          NOME_LOCAL: this.addNomeLocal,
          NOME_ABREVIADO: this.addNomeAbrev,
          NOME_ATUALIZ: "Luiza"
        });
    ...
    async fetchData() {
      this.isLoading = true;
      try {
        const response = await axios.get('http://localhost:3000/LOCAL_ALL');
        this.data = response.data;
        this.frmLocalTable = this.data;
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
      } finally {
    ...
    async deleteFromTable() {
      this.isLoading = true;
      try {
        const response = await axios.delete(`http://localhost:3000/LOCAL_DELETE/${this.selectedRow.LOCAL}`);
        console.log(response.data);
        await this.fetchData();
    ...
    async saveUpdate() {
      ...
      this.isEditing = false;
      try {
        const response = await axios.post(`http://localhost:3000/LOCAL_UPDATE`, {
          LOCAL: this.selectedRow.LOCAL,
          NOME_LOCAL: this.selectedRow.NOME_LOCAL,
          NOME_ABREVIADO:
            this.selectedRow.NOME_ABREVIADO,
          NOME_ATUALIZ: "Luiza"
        });

        if (response.status === 200) {
          this.frmLocalTable = this.frmLocalTable.map(row => row.LOCAL_LOGRAD === this.selectedRow.LOCAL_LOGRAD ? this.selectedRow : row);
          this.isEditing = false;
        } else {
