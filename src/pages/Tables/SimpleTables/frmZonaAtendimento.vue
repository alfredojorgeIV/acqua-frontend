      ...
      <div class="content">
        <section>
          <div class="intro-services-to-be-billed">
            <h1 ref="title" class="title-page">Zona de Atendimento</h1>
          </div>
          <div class="flex gap-4">
            <div class="w-61 margin-top-30">
              ...
              <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
                :value="frmZonaAtendimentoTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10"
                dataKey="ZA_CODIGO" @row-click="handleRowClick" :rowClass="rowClass">
            ...
            <div class="w-35">
              <div class="box-tables-info" v-if="selectedRow && !showAddBox">
                ...
                <input v-model="selectedRow.ZA_CODIGO" maxlength="5" class="inputs margin-bottom-30"
                  :disabled="!isEditing" />
                <p class="p-labels">Descrição</p>
                <input v-model="selectedRow.ZA_DESCRICAO" maxlength="15" class="inputs margin-bottom-30"
                  :disabled="!isEditing" />
              ...
              <div class="box-tables-info" v-if="showAddBox && !selectedRow">
                <p class="p-labels">Código</p>
                <input class="inputs margin-bottom-30" maxlength="5" v-model="addZaCodigo" />
                <p class="p-labels">Descrição</p>
                <input class="inputs margin-bottom-30" maxlength="15" v-model="addZaDescricao" />
                <div class="flex justify-center gap-4 mt-40">
  ...
  export default {
    ...
    methods: {
      ...
      rowClass(data) {
        return {
          'pointer-cursor': true,
          'active-row': this.selectedRow && this.selectedRow.ZA_CODIGO === data.ZA_CODIGO
        };
      },
      ...
      async addToTable() {
        ...
        try {
          const response = await axios.post(`${baseUrl}/ZONA_ATENDIMENTO_ADD`, {
            ZA_CODIGO: Number(this.addZaCodigo),
            ZA_DESCRICAO: this.addZaDescricao,
      ...
      async fetchData() {
        ...
        const baseUrl = user.databaseUrl || 'http://localhost:3000';
        try {
          const response = await axios.get(`${baseUrl}/ZONA_ATENDIMENTO_ALL`);
          this.data = response.data;
          this.frmZonaAtendimentoTable = this.data;
        } catch (error) {
          console.error('Erro:', error);
          this.error = error.toString();
        } finally {
      ...
      async deleteFromTable() {
        ...
        const baseUrl = user.databaseUrl || 'http://localhost:3000';
        try {
          const response = await axios.delete(`${baseUrl}/ZONA_ATENDIMENTO_DELETE/${this.selectedRow.ZA_CODIGO}/${user.username}`);
          console.log(response.data);
          await this.fetchData();
      ...
      async saveUpdate() {
        ...
        const baseUrl = user.databaseUrl || 'http://localhost:3000';
        try {
          const response = await axios.post(`${baseUrl}/ZONA_ATENDIMENTO_UPDATE`, {
            ZA_CODIGO: Number(this.selectedRow.ZA_CODIGO),
            ZA_DESCRICAO: this.selectedRow.ZA_DESCRICAO,
            NOME_ATUALIZ: user.username
          });
  
          if (response.status === 200) {
            this.frmZonaAtendimentoTable = this.frmZonaAtendimentoTable.map(row =>
              row.ZA_CODIGO === this.selectedRow.ZA_CODIGO ? this.selectedRow : row
            );
            this.isEditing = false;
          } else {
      ...
      printPDF() {
        ...
        doc.autoTable({
          ...
          body: this.frmZonaAtendimentoTable.map(row => this.columns.map(col => row[col.field])),
          theme: 'grid',
        });
