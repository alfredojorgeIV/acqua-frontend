    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Fonte Alternativa</h1>
        </div>
        <div class="flex gap-4">
          <div class="w-61 margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmFonteTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10"
              dataKey="FONTE" @row-click="handleRowClick" :rowClass="rowClass">
          ...
          <div class="w-35">
            <div class="box-tables-info" v-if="selectedRow && !showAddBox">
              ...
              <input v-model="selectedRow.FONTE" maxlength="5" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
              <p class="p-labels">Descrição</p>
              <input v-model="selectedRow.DESC_FONTE" maxlength="15" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
            ...
            <div class="box-tables-info" v-if="showAddBox && !selectedRow">
              <p class="p-labels">Código</p>
              <input class="inputs margin-bottom-30" maxlength="5" v-model="addFonte" />
              <p class="p-labels">Descrição</p>
              <input class="inputs margin-bottom-30" maxlength="15" v-model="addDescFonte" />
              <div class="flex justify-center gap-4 mt-40">
...
export default {
  ...
  methods: {
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.FONTE === data.FONTE
      };
    },
    ...
    async addToTable() {
      ...
      try {
        const response = await axios.post(`${baseUrl}/FONTE_ALTERNATIVA_ADD`, {
          FONTE: Number(this.addFonte),
          DESC_FONTE: this.addDescFonte,
    ...
    async fetchData() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.get(`${baseUrl}/FONTE_ALTERNATIVA_ALL`);
        this.data = response.data;
        this.frmFonteTable = this.data;
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
      } finally {
    ...
    async deleteFromTable() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.delete(`${baseUrl}/FONTE_ALTERNATIVA_DELETE/${this.selectedRow.FONTE}/${user.username}`);
        console.log(response.data);
        await this.fetchData();
    ...
    async saveUpdate() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.post(`${baseUrl}/FONTE_ALTERNATIVA_UPDATE`, {
          FONTE: Number(this.selectedRow.FONTE),
          DESC_FONTE: this.selectedRow.DESC_FONTE,
          NOME_ATUALIZ: user.username
        });

        if (response.status === 200) {
          this.frmFonteTable = this.frmFonteTable.map(row =>
            row.FONTE === this.selectedRow.FONTE ? this.selectedRow : row
          );
          this.isEditing = false;
        } else {
          console.error("Erro ao atualizar a Fonte Alternativa");
        }
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmFonteTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
      });
