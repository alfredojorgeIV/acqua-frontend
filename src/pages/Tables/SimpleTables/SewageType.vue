    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 class="title-page">Tipo de esgotamento</h1>
        </div>
        <div class="flex gap-4">
          <div class="w-61 margin-top-30">
            ...
            <DataTable style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="sewageTypeTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="id"
          ...
          <div class="w-35">
            <div class="box-tables-info" v-if="selectedRow && !showAddBox">
              ...
              <input v-model="selectedRow.TIPO_ESGOTAMENTO" class="inputs margin-bottom-30" :disabled="!isEditing" />
              <p class="p-labels">Descrição</p>
              <input v-model="selectedRow.DESC_ESGOT" class="inputs margin-bottom-30" :disabled="!isEditing" />
            ...
            <div class="box-tables-info" v-if="showAddBox && !selectedRow">
              <p class="p-labels">Código</p>
              <input class="inputs margin-bottom-30" v-model="addTIPO_ESGOTAMENTO" />
              <p class="p-labels">Descrição</p>
              <input class="inputs margin-bottom-30" v-model="addDescricao" />

              <div class="flex justify-center gap-4 mt-40">
                <Button class="primary-button flex align-center" @click="addTIPO_ESGOTAMENTOTarifa">
...
export default {
  ...
  methods: {
    handleAddRowClick(event) {
      ...
      this.addTIPO_ESGOTAMENTO = ''; // Corrected
    ...
    async addTIPO_ESGOTAMENTOTarifa() {
      this.isLoading = true;
      try {
        const response = await axios.post('http://localhost:3000/TIPO_ESGOTAMENTO_ADD', {
          TIPO_ESGOTAMENTO: Number(this.addTIPO_ESGOTAMENTO), // Corrected
          DESC_ESGOT: this.addDescricao,
          NOME_ATUALIZ: "Luiza"
        });
    ...
    async fetchData() {
      this.isLoading = true;
      try {
        const response = await axios.get('http://localhost:3000/TIPO_ESGOTAMENTO_ALL');
        this.data = response.data;
        this.sewageTypeTable = this.data;
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
      } finally {
    ...
    async deleteTaxCategory() {
      this.isLoading = true;
      try {
        const response = await axios.delete(`http://localhost:3000/TIPO_ESGOTAMENTO_DELETE/${this.selectedRow.TIPO_ESGOTAMENTO}`);
        console.log(response.data);
        await this.fetchData();
    ...
    async saveUpdate() {
      this.isLoading = true;
      try {
        const response = await axios.post(`http://localhost:3000/TIPO_ESGOTAMENTO_UPDATE`, {
          TIPO_ESGOTAMENTO: this.selectedRow.TIPO_ESGOTAMENTO,
          DESC_ESGOT: this.selectedRow.DESC_ESGOT,
          NOME_ATUALIZ: "Luiza"
        });

        if (response.status === 200) {
          this.sewageTypeTable = this.sewageTypeTable.map(row => row.TIPO_ESGOTAMENTO === this.selectedRow.TIPO_ESGOTAMENTO ? this.selectedRow : row);
          this.isEditing = false;
        } else {
          console.error("Erro ao atualizar a TIPO_ESGOTAMENTO de tarifa");
        }
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
    ...
    cancelAdd() {
      this.showAddBox = false;
      this.addTIPO_ESGOTAMENTO = ''; // Clear input
