    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 class="title-page">Tipo de logradouro</h1>
        </div>
        <div class="flex gap-4">
          <div class="w-61 margin-top-30">
            ...
            <DataTable style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="streetTypeTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="id"
          ...
          <div class="w-35">
            <div class="box-tables-info" v-if="selectedRow && !showAddBox">
              ...
              <input v-model="selectedRow.TIPO_LOGRAD" class="inputs margin-bottom-30" :disabled="!isEditing" />
              <p class="p-labels">Descrição</p>
              <input v-model="selectedRow.DESC_TIPO_LOGRAD" class="inputs margin-bottom-30" :disabled="!isEditing" />
            ...
            <div class="box-tables-info" v-if="showAddBox && !selectedRow">
              <p class="p-labels">Código</p>
              <input class="inputs margin-bottom-30" v-model="addTIPO_LOGRAD" />
              <p class="p-labels">Descrição</p>
              <input class="inputs margin-bottom-30" v-model="addDescricao" />

              <div class="flex justify-center gap-4 mt-40">
                <Button class="primary-button flex align-center" @click="addTIPO_LOGRADTarifa">
...
export default {
  ...
  methods: {
    ...
    async addTIPO_LOGRADTarifa() {
      this.isLoading = true;
      try {
        const response = await axios.post('http://localhost:3000/TIPO_LOGRADOURO_ADD', {
          TIPO_LOGRAD: Number(this.addTIPO_LOGRAD), // Corrected
          DESC_TIPO_LOGRAD: this.addDescricao,
          NOME_ATUALIZ: "Luiza"
        });
    ...
    async fetchData() {
      this.isLoading = true;
      try {
        const response = await axios.get('http://localhost:3000/TIPO_LOGRADOURO_ALL');
        this.data = response.data;
        this.streetTypeTable = this.data;
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
      } finally {
    ...
    async deleteTaxCategory() {
      this.isLoading = true;
      try {
        const response = await axios.delete(`http://localhost:3000/TIPO_LOGRADOURO_DELETE/${this.selectedRow.TIPO_LOGRAD}`);
        console.log(response.data);
        await this.fetchData();
    ...
    async saveUpdate() {
      this.isLoading = true;
      try {
        const response = await axios.post(`http://localhost:3000/TIPO_LOGRADOURO_UPDATE`, {
        ...
        if (response.status === 200) {
          this.streetTypeTable = this.streetTypeTable.map(row => row.TIPO_LOGRAD === this.selectedRow.TIPO_LOGRAD ? this.selectedRow : row);
          this.isEditing = false;
        } else {
          console.error("Erro ao atualizar a TIPO_LOGRAD de tarifa");
        }
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
    ...
    cancelAdd() {
      this.showAddBox = false;
      this.addTIPO_LOGRAD = ''; // Clear input
