        ...
        <div class="flex gap-4">
          ...
          <div class="w-35">
            <div class="box-tables-info" v-if="selectedRow && !showAddBox">
              <p class="p-labels">Código do funcionário</p>
              <input
                v-model="selectedRow.MARCA_HD"
                maxlength="5"
              ...
              <input
                v-model="selectedRow.DESC_MARCA_HD"
                maxlength="15"
                class="inputs margin-bottom-30"
                :disabled="!isEditing"
              />
            ...
            <div class="box-tables-info" v-if="showAddBox && !selectedRow">
              ...
              <input
                class="inputs margin-bottom-30"
                maxlength="15"
                v-model="addDescMarcaHd"
              />
              <p class="p-labels">Marca reduzida</p>
              <input
                class="inputs margin-bottom-30"
                maxlength="15"
                v-model="addMarcaRed"
  ...
  methods: {
    ...
    rowClass(data) {
      return {
        "pointer-cursor": true,
        "active-row":
          this.selectedRow && this.selectedRow.MARCA_HD === data.MARCA_HD,
      };
    },
    ...
    async addToTable() {
      ...
      try {
        const response = await axios.post(`${baseUrl}/MARCA_HD_ADD`, {
          MARCA_HD: Number(this.addMarcaHd),
          DESC_MARCA_HD: this.addDescMarcaHd,
          MARCA_RED: this.addMarcaRed,
    ...
    async fetchData() {
      ...
      const baseUrl = user.databaseUrl || "http://localhost:3000";
      try {
        const response = await axios.get(`${baseUrl}/MARCA_HD_ALL`);
        this.data = response.data;
        this.frmMarcaTable = this.data;
      } catch (error) {
        console.error("Erro:", error);
        this.error = error.toString();
      } finally {
    ...
    async deleteFromTable() {
      ...
      try {
        const response = await axios.delete(
          `${baseUrl}/MARCA_HD_DELETE/${this.selectedRow.MARCA_HD}/${user.username}`
        );
        console.log(response.data);
        await this.fetchData();
    ...
    async saveUpdate() {
      ...
      const baseUrl = user.databaseUrl || "http://localhost:3000";
      try {
        const response = await axios.post(`${baseUrl}/MARCA_HD_UPDATE`, {
          MARCA_HD: Number(this.selectedRow.MARCA_HD),
          DESC_MARCA_HD: this.selectedRow.DESC_MARCA_HD,
          MARCA_RED: this.selectedRow.MARCA_RED,
          NOME_ATUALIZ: user.username,
        });

        if (response.status === 200) {
          this.frmMarcaTable = this.frmMarcaTable.map((row) =>
            row.MARCA_HD === this.selectedRow.MARCA_HD
              ? this.selectedRow
              : row
          );
          this.isEditing = false;
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmMarcaTable.map((row) =>
