      ...
      <div class="content">
        <section>
          <div class="intro-services-to-be-billed">
            <h1 ref="title" class="title-page">Motivo do Encerramento</h1>
          </div>
          <div class="flex gap-4">
            <div class="w-61 margin-top-30">
              ...
              <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
                :value="frmMotivoEncerramentoTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10"
                dataKey="MT_ENCERRAMENTO" @row-click="handleRowClick" :rowClass="rowClass">
            ...
            <div class="w-35">
              <div class="box-tables-info" v-if="selectedRow && !showAddBox">
                ...
                <input v-model="selectedRow.MT_ENCERRAMENTO" maxlength="5" class="inputs margin-bottom-30"
                  :disabled="!isEditing" />
                <p class="p-labels">Descrição</p>
                <input v-model="selectedRow.DESC_MT_ENCERRAMENTO" maxlength="15" class="inputs margin-bottom-30"
                  :disabled="!isEditing" />
              ...
              <div class="box-tables-info" v-if="showAddBox && !selectedRow">
                <p class="p-labels">Código</p>
                <input class="inputs margin-bottom-30" maxlength="5" v-model="addMtEncerramento" />
                <p class="p-labels">Descrição</p>
                <input class="inputs margin-bottom-30" maxlength="15" v-model="addDescMtEncerramento" />
                <div class="flex justify-center gap-4 mt-40">
  ...
  export default {
    ...
    data() {
      return {
        ...
        selectedRow: null,
        addMtEncerramento: '',
        addDescMtEncerramento: '',
        columns: [
          {
            name: "MT_ENCERRAMENTO",
            required: true,
            label: "Código",
            align: "left",
            field: "MT_ENCERRAMENTO",
          },
          {
            name: "DESC_MT_ENCERRAMENTO",
            required: true,
            label: "Descrição",
            align: "left",
            field: "DESC_MT_ENCERRAMENTO",
          },
        ],
        frmMotivoEncerramentoTable: []
      };
    },
    ...
    methods: {
      handleAddRowClick(event) {
        ...
        this.addMtEncerramento = '';
        this.addDescMtEncerramento = '';
      },
      ...
      rowClass(data) {
        return {
          'pointer-cursor': true,
          'active-row': this.selectedRow && this.selectedRow.MT_ENCERRAMENTO === data.MT_ENCERRAMENTO
        };
      },
      ...
      async addToTable() {
        ...
        try {
          const response = await axios.post(`${baseUrl}/MOTIVO_ENCERRAMENTO_ADD`, {
            MT_ENCERRAMENTO: Number(this.addMtEncerramento),
            DESC_MT_ENCERRAMENTO: this.addDescMtEncerramento,
      ...
      async fetchData() {
        ...
        const baseUrl = user.databaseUrl || 'http://localhost:3000';
        try {
          const response = await axios.get(`${baseUrl}/MOTIVO_ENCERRAMENTO_ALL`);
          this.data = response.data;
          this.frmMotivoEncerramentoTable = this.data;
        } catch (error) {
          console.error('Erro:', error);
          this.error = error.toString();
        } finally {
      ...
      async deleteFromTable() {
        ...
        const baseUrl = user.databaseUrl || 'http://localhost:3000';
        try {
          const response = await axios.delete(`${baseUrl}/MOTIVO_ENCERRAMENTO_DELETE/${this.selectedRow.MT_ENCERRAMENTO}/${user.username}`);
          console.log(response.data);
          await this.fetchData();
      ...
      async saveUpdate() {
        ...
        const baseUrl = user.databaseUrl || 'http://localhost:3000';
        try {
          const response = await axios.post(`${baseUrl}/MOTIVO_ENCERRAMENTO_UPDATE`, {
            MT_ENCERRAMENTO: Number(this.selectedRow.MT_ENCERRAMENTO),
            DESC_MT_ENCERRAMENTO: this.selectedRow.DESC_MT_ENCERRAMENTO,
            NOME_ATUALIZ: user.username
          });
  
          if (response.status === 200) {
            this.frmMotivoEncerramentoTable = this.frmMotivoEncerramentoTable.map(row =>
              row.MT_ENCERRAMENTO === this.selectedRow.MT_ENCERRAMENTO ? this.selectedRow : row
            );
            this.isEditing = false;
          } else {
            console.error("Erro ao atualizar o Motivo do Encerramento");
          }
        } catch (error) {
          console.error('Erro:', error);
          this.error = error.toString();
      ...
      printPDF() {
        ...
        doc.autoTable({
          ...
          body: this.frmMotivoEncerramentoTable.map(row => this.columns.map(col => row[col.field])),
          theme: 'grid',
        });
