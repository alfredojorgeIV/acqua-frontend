        ...
        <div class="flex gap-4">
          <div class="w-61 margin-top-30">
            ...
            <DataTable style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="categoryTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="id"
          ...
          <div class="w-35">
            <div class="box-tables-info" v-if="selectedRow && !showAddBox">
              ...
              <input v-model="selectedRow.DESC_CATEGORIA" class="inputs margin-bottom-30" :disabled="!isEditing" />
...
export default {
  ...
  methods: {
    ...
    async addCategoriaTarifa() {
      this.isLoading = true;
      try {
        const response = await axios.post('http://localhost:3000/CATEGORIA_ADD', {
    ...
    async fetchData() {
      this.isLoading = true;
      try {
        const response = await axios.get('http://localhost:3000/CATEGORIA_ALL');
        this.data = response.data;
        this.categoryTable = this.data;
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
      } finally {
    ...
    async deleteTaxCategory() {
      this.isLoading = true;
      try {
        const response = await axios.delete(`http://localhost:3000/CATEGORIA_DELETE/${this.selectedRow.CATEGORIA}`);
        console.log(response.data);
        await this.fetchData();
    ...
    async saveUpdate() {
      this.isLoading = true;
      try {
        const response = await axios.post(`http://localhost:3000/CATEGORIA_UPDATE`, {
        ...
        if (response.status === 200) {
          this.categoryTable = this.categoryTable.map(row => row.CATEGORIA === this.selectedRow.CATEGORIA ? this.selectedRow : row);
          this.isEditing = false;
        } else {
