      ...
      <div class="content">
        <section>
          <div class="intro-services-to-be-billed">
            <h1 ref="title" class="title-page">Tipo de Consumidor Especial</h1>
          </div>
          <div class="flex gap-4">
            <div class="w-61 margin-top-30">
              ...
              <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
                :value="frmTipoConsEspTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10"
                dataKey="TE_CODIGO" @row-click="handleRowClick" :rowClass="rowClass">
            ...
            <div class="w-35">
              <div class="box-tables-info" v-if="selectedRow && !showAddBox">
                ...
                <input v-model="selectedRow.TE_CODIGO" maxlength="5" class="inputs margin-bottom-30"
                  :disabled="!isEditing" />
                <p class="p-labels">Descrição</p>
                <input v-model="selectedRow.TE_DESCRICAO" maxlength="15" class="inputs margin-bottom-30"
                  :disabled="!isEditing" />
              ...
              <div class="box-tables-info" v-if="showAddBox && !selectedRow">
                <p class="p-labels">Código</p>
                <input class="inputs margin-bottom-30" maxlength="5" v-model="addMvCodigo" />
                <p class="p-labels">Descrição</p>
                <input class="inputs margin-bottom-30" maxlength="15" v-model="addMvDescricao" />
                <div class="flex justify-center gap-4 mt-40">
  ...
  export default {
    ...
    methods: {
      ...
      rowClass(data) {
        return {
          'pointer-cursor': true,
          'active-row': this.selectedRow && this.selectedRow.TE_CODIGO === data.TE_CODIGO
        };
      },
      ...
      async addToTable() {
        ...
        try {
          const response = await axios.post(`${baseUrl}/TIPO_CONSUMIDOR_ESPECIAL_ADD`, {
            TE_CODIGO: Number(this.addMvCodigo),
            TE_DESCRICAO: this.addMvDescricao,
      ...
      async fetchData() {
        ...
        const baseUrl = user.databaseUrl || 'http://localhost:3000';
        try {
          const response = await axios.get(`${baseUrl}/TIPO_CONSUMIDOR_ESPECIAL_ALL`);
          this.data = response.data;
          this.frmTipoConsEspTable = this.data;
        } catch (error) {
          console.error('Erro:', error);
          this.error = error.toString();
        } finally {
      ...
      async deleteFromTable() {
        ...
        const baseUrl = user.databaseUrl || 'http://localhost:3000';
        try {
          const response = await axios.delete(`${baseUrl}/TIPO_CONSUMIDOR_ESPECIAL_DELETE/${this.selectedRow.TE_CODIGO}/${user.username}`);
          console.log(response.data);
          await this.fetchData();
      ...
      async saveUpdate() {
        ...
        const baseUrl = user.databaseUrl || 'http://localhost:3000';
        try {
          const response = await axios.post(`${baseUrl}/TIPO_CONSUMIDOR_ESPECIAL_UPDATE`, {
            TE_CODIGO: Number(this.selectedRow.TE_CODIGO),
            TE_DESCRICAO: this.selectedRow.TE_DESCRICAO,
            NOME_ATUALIZ: user.username
          });
  
          if (response.status === 200) {
            this.frmTipoConsEspTable = this.frmTipoConsEspTable.map(row =>
              row.TE_CODIGO === this.selectedRow.TE_CODIGO ? this.selectedRow : row
            );
            this.isEditing = false;
          } else {
      ...
      printPDF() {
        ...
        doc.autoTable({
          ...
          body: this.frmTipoConsEspTable.map(row => this.columns.map(col => row[col.field])),
          theme: 'grid',
        });
