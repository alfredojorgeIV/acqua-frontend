          ...
          <div class="flex gap-4">
            <div class="w-61 margin-top-30">
              ...
              <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
                :value="frmMotivoInativoTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10"
                dataKey="MT_INATIVO" @row-click="handleRowClick" :rowClass="rowClass">
            ...
            <div class="w-35">
              <div class="box-tables-info" v-if="selectedRow && !showAddBox">
                ...
                <input v-model="selectedRow.MT_INATIVO" maxlength="5" class="inputs margin-bottom-30"
                  :disabled="!isEditing" />
                <p class="p-labels">Descrição</p>
                <input v-model="selectedRow.DESC_INATIVO" maxlength="15" class="inputs margin-bottom-30"
                  :disabled="!isEditing" />
              ...
              <div class="box-tables-info" v-if="showAddBox && !selectedRow">
                <p class="p-labels">Código</p>
                <input class="inputs margin-bottom-30" maxlength="5" v-model="addMtInativo" />
                <p class="p-labels">Descrição</p>
                <input class="inputs margin-bottom-30" maxlength="15" v-model="addDescInativo" />
                <div class="flex justify-center gap-4 mt-40">
  ...
  export default {
    ...
    methods: {
      ...
      rowClass(data) {
        return {
          'pointer-cursor': true,
          'active-row': this.selectedRow && this.selectedRow.MT_INATIVO === data.MT_INATIVO
        };
      },
      ...
      async addToTable() {
        ...
        try {
          const response = await axios.post(`${baseUrl}/MOTIVO_INATIVO_ADD`, {
            MT_INATIVO: Number(this.addMtInativo),
            DESC_INATIVO: this.addDescInativo,
      ...
      async fetchData() {
        ...
        const baseUrl = user.databaseUrl || 'http://localhost:3000';
        try {
          const response = await axios.get(`${baseUrl}/MOTIVO_INATIVO_ALL`);
          this.data = response.data;
          this.frmMotivoInativoTable = this.data;
        } catch (error) {
          console.error('Erro:', error);
          this.error = error.toString();
        } finally {
      ...
      async deleteFromTable() {
        ...
        const baseUrl = user.databaseUrl || 'http://localhost:3000';
        try {
          const response = await axios.delete(`${baseUrl}/MOTIVO_INATIVO_DELETE/${this.selectedRow.MT_INATIVO}/${user.username}`);
          console.log(response.data);
          await this.fetchData();
      ...
      async saveUpdate() {
        ...
        const baseUrl = user.databaseUrl || 'http://localhost:3000';
        try {
          const response = await axios.post(`${baseUrl}/MOTIVO_INATIVO_UPDATE`, {
            MT_INATIVO: Number(this.selectedRow.MT_INATIVO),
            DESC_INATIVO: this.selectedRow.DESC_INATIVO,
            NOME_ATUALIZ: user.username
          });
  
          if (response.status === 200) {
            this.frmMotivoInativoTable = this.frmMotivoInativoTable.map(row =>
              row.MT_INATIVO === this.selectedRow.MT_INATIVO ? this.selectedRow : row
            );
            this.isEditing = false;
          } else {
      ...
      printPDF() {
        ...
        doc.autoTable({
          ...
          body: this.frmMotivoInativoTable.map(row => this.columns.map(col => row[col.field])),
          theme: 'grid',
        });
