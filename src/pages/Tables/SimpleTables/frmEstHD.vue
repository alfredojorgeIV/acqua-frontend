    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Estado do HD</h1>
        </div>
        <div class="flex gap-4">
          <div class="w-61 margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmEstHDTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10"
              dataKey="ESTADO_HD" @row-click="handleRowClick" :rowClass="rowClass">
          ...
          <div class="w-35">
            <div class="box-tables-info" v-if="selectedRow && !showAddBox">
              ...
              <input v-model="selectedRow.ESTADO_HD" maxlength="5" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
              <p class="p-labels">Descrição</p>
              <input v-model="selectedRow.DESC_ESTADO_HD" maxlength="15" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
            ...
            <div class="box-tables-info" v-if="showAddBox && !selectedRow">
              <p class="p-labels">Código</p>
              <input class="inputs margin-bottom-30" maxlength="5" v-model="addEstadoHd" />
              <p class="p-labels">Descrição</p>
              <input class="inputs margin-bottom-30" maxlength="15" v-model="addDescEstadoHd" />
              <div class="flex justify-center gap-4 mt-40">
...
export default {
  ...
  methods: {
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.ESTADO_HD === data.ESTADO_HD
      };
    },
    ...
    async addToTable() {
      ...
      try {
        const response = await axios.post(`${baseUrl}/ESTADO_HIDROMETRO_ADD`, {
          ESTADO_HD: Number(this.addEstadoHd),
          DESC_ESTADO_HD: this.addDescEstadoHd,
    ...
    async fetchData() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.get(`${baseUrl}/ESTADO_HIDROMETRO_ALL`);
        this.data = response.data;
        this.frmEstHDTable = this.data;
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
      } finally {
    ...
    async deleteFromTable() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.delete(`${baseUrl}/ESTADO_HIDROMETRO_DELETE/${this.selectedRow.ESTADO_HD}/${user.username}`);
        console.log(response.data);
        await this.fetchData();
    ...
    async saveUpdate() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.post(`${baseUrl}/ESTADO_HIDROMETRO_UPDATE`, {
          ESTADO_HD: Number(this.selectedRow.ESTADO_HD),
          DESC_ESTADO_HD: this.selectedRow.DESC_ESTADO_HD,
          NOME_ATUALIZ: user.username
        });

        if (response.status === 200) {
          this.frmEstHDTable = this.frmEstHDTable.map(row =>
            row.ESTADO_HD === this.selectedRow.ESTADO_HD ? this.selectedRow : row
          );
          this.isEditing = false;
        } else {
          console.error("Erro ao atualizar o Estado do HD");
        }
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmEstHDTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
      });
