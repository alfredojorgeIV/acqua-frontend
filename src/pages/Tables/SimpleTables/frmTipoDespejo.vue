    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Tipo de Despejo</h1>
        </div>
        <div class="flex gap-4">
          <div class="w-61 margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmTipoDespejoTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10"
              dataKey="TIPO_DESPEJO" @row-click="handleRowClick" :rowClass="rowClass">
          ...
          <div class="w-35">
            <div class="box-tables-info" v-if="selectedRow && !showAddBox">
              ...
              <input v-model="selectedRow.TIPO_DESPEJO" maxlength="5" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
              <p class="p-labels">Descrição</p>
              <input v-model="selectedRow.DESC_DESPEJO" maxlength="15" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
            ...
            <div class="box-tables-info" v-if="showAddBox && !selectedRow">
              <p class="p-labels">Código</p>
              <input class="inputs margin-bottom-30" maxlength="5" v-model="addTipoDespejo" />
              <p class="p-labels">Descrição</p>
              <input class="inputs margin-bottom-30" maxlength="15" v-model="addDescDespejo" />
              <div class="flex justify-center gap-4 mt-40">
...
export default {
  ...
  methods: {
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.TIPO_DESPEJO === data.TIPO_DESPEJO
      };
    },
    ...
    async addToTable() {
      ...
      try {
        const response = await axios.post(`${baseUrl}/TIPO_DESPEJO_ADD`, {
          TIPO_DESPEJO: Number(this.addTipoDespejo),
          DESC_DESPEJO: this.addDescDespejo,
    ...
    async fetchData() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.get(`${baseUrl}/TIPO_DESPEJO_ALL`);
        this.data = response.data;
        this.frmTipoDespejoTable = this.data;
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
      } finally {
    ...
    async deleteFromTable() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.delete(`${baseUrl}/TIPO_DESPEJO_DELETE/${this.selectedRow.TIPO_DESPEJO}/${user.username}`);
        console.log(response.data);
        await this.fetchData();
    ...
    async saveUpdate() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.post(`${baseUrl}/TIPO_DESPEJO_UPDATE`, {
          TIPO_DESPEJO: Number(this.selectedRow.TIPO_DESPEJO),
          DESC_DESPEJO: this.selectedRow.DESC_DESPEJO,
          NOME_ATUALIZ: user.username
        });

        if (response.status === 200) {
          this.frmTipoDespejoTable = this.frmTipoDespejoTable.map(row =>
            row.TIPO_DESPEJO === this.selectedRow.TIPO_DESPEJO ? this.selectedRow : row
          );
          this.isEditing = false;
        } else {
          console.error("Erro ao atualizar o Tipo de Despejo");
        }
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmTipoDespejoTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
      });
