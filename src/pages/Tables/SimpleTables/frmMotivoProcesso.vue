      ...
      <div class="content">
        <section>
          <div class="intro-services-to-be-billed">
            <h1 ref="title" class="title-page">Motivo do Processo</h1>
          </div>
          <div class="flex gap-4">
            <div class="w-61 margin-top-30">
              ...
              <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
                :value="frmMotivoProcessoTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10"
                dataKey="MOTIVO_PROCESSO" @row-click="handleRowClick" :rowClass="rowClass">
            ...
            <div class="w-35">
              <div class="box-tables-info" v-if="selectedRow && !showAddBox">
                ...
                <input v-model="selectedRow.MOTIVO_PROCESSO" maxlength="5" class="inputs margin-bottom-30"
                  :disabled="!isEditing" />
                <p class="p-labels">Descrição</p>
                <input v-model="selectedRow.DESC_MOTIVO" maxlength="15" class="inputs margin-bottom-30"
                  :disabled="!isEditing" />
              ...
              <div class="box-tables-info" v-if="showAddBox && !selectedRow">
                <p class="p-labels">Código</p>
                <input class="inputs margin-bottom-30" maxlength="5" v-model="addMotivoProcesso" />
                <p class="p-labels">Descrição</p>
                <input class="inputs margin-bottom-30" maxlength="15" v-model="addDescMotivo" />
                <div class="flex justify-center gap-4 mt-40">
  ...
  export default {
    ...
    data() {
      return {
        ...
        columns: [
          {
            name: "MOTIVO_PROCESSO",
            required: true,
            label: "Código",
            align: "left",
            field: "MOTIVO_PROCESSO",
          },
          {
            name: "DESC_MOTIVO",
            required: true,
    ...
    methods: {
      ...
      rowClass(data) {
        return {
          'pointer-cursor': true,
          'active-row': this.selectedRow && this.selectedRow.MOTIVO_PROCESSO === data.MOTIVO_PROCESSO
        };
      },
      ...
      async addToTable() {
        ...
        try {
          const response = await axios.post(`${baseUrl}/MOTIVO_PROCESSO_ADD`, {
            MOTIVO_PROCESSO: Number(this.addMotivoProcesso),
            DESC_MOTIVO: this.addDescMotivo,
      ...
      async fetchData() {
        ...
        const baseUrl = user.databaseUrl || 'http://localhost:3000';
        try {
          const response = await axios.get(`${baseUrl}/MOTIVO_PROCESSO_ALL`);
          this.data = response.data;
          this.frmMotivoProcessoTable = this.data;
        } catch (error) {
          console.error('Erro:', error);
          this.error = error.toString();
        } finally {
      ...
      async deleteFromTable() {
        ...
        const baseUrl = user.databaseUrl || 'http://localhost:3000';
        try {
          const response = await axios.delete(`${baseUrl}/MOTIVO_PROCESSO_DELETE/${this.selectedRow.MOTIVO_PROCESSO}/${user.username}`);
          console.log(response.data);
          await this.fetchData();
      ...
      async saveUpdate() {
        ...
        const baseUrl = user.databaseUrl || 'http://localhost:3000';
        try {
          const response = await axios.post(`${baseUrl}/MOTIVO_PROCESSO_UPDATE`, {
            MOTIVO_PROCESSO: Number(this.selectedRow.MOTIVO_PROCESSO),
            DESC_MOTIVO: this.selectedRow.DESC_MOTIVO,
            NOME_ATUALIZ: user.username
          });
  
          if (response.status === 200) {
            this.frmMotivoProcessoTable = this.frmMotivoProcessoTable.map(row =>
              row.MOTIVO_PROCESSO === this.selectedRow.MOTIVO_PROCESSO ? this.selectedRow : row
            );
            this.isEditing = false;
          } else {
            console.error("Erro ao atualizar o Motivo do Processo");
          }
        } catch (error) {
          console.error('Erro:', error);
          this.error = error.toString();
      ...
      printPDF() {
        ...
        doc.autoTable({
          ...
          body: this.frmMotivoProcessoTable.map(row => this.columns.map(col => row[col.field])),
          theme: 'grid',
        });
