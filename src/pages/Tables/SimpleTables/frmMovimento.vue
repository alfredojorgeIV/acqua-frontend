      ...
      <div class="content">
        <section>
          <div class="intro-services-to-be-billed">
            <h1 ref="title" class="title-page">Movimento</h1>
          </div>
          <div class="flex gap-4">
            <div class="w-61 margin-top-30">
              ...
              <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
                :value="frmMovimentoTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10"
                dataKey="MV_CODIGO" @row-click="handleRowClick" :rowClass="rowClass">
            ...
            <div class="w-35">
              <div class="box-tables-info" v-if="selectedRow && !showAddBox">
                ...
                <input v-model="selectedRow.MV_CODIGO" maxlength="5" class="inputs margin-bottom-30"
                  :disabled="!isEditing" />
                <p class="p-labels">Descrição</p>
                <input v-model="selectedRow.MV_DESCRICAO" maxlength="15" class="inputs margin-bottom-30"
                  :disabled="!isEditing" />
  ...
  export default {
    ...
    methods: {
      ...
      rowClass(data) {
        return {
          'pointer-cursor': true,
          'active-row': this.selectedRow && this.selectedRow.MV_CODIGO === data.MV_CODIGO
        };
      },
      ...
      async addToTable() {
        ...
        try {
          const response = await axios.post(`${baseUrl}/MOVIMENTO_ADD`, {
            MV_CODIGO: Number(this.addMvCodigo),
            MV_DESCRICAO: this.addMvDescricao,
      ...
      async fetchData() {
        ...
        const baseUrl = user.databaseUrl || 'http://localhost:3000';
        try {
          const response = await axios.get(`${baseUrl}/MOVIMENTO_ALL`);
          this.data = response.data;
          this.frmMovimentoTable = this.data;
        } catch (error) {
          console.error('Erro:', error);
          this.error = error.toString();
        } finally {
      ...
      async deleteFromTable() {
        ...
        const baseUrl = user.databaseUrl || 'http://localhost:3000';
        try {
          const response = await axios.delete(`${baseUrl}/MOVIMENTO_DELETE/${this.selectedRow.MV_CODIGO}/${user.username}`);
          console.log(response.data);
          await this.fetchData();
      ...
      async saveUpdate() {
        ...
        const baseUrl = user.databaseUrl || 'http://localhost:3000';
        try {
          const response = await axios.post(`${baseUrl}/MOVIMENTO_UPDATE`, {
            MV_CODIGO: Number(this.selectedRow.MV_CODIGO),
            MV_DESCRICAO: this.selectedRow.MV_DESCRICAO,
            NOME_ATUALIZ: user.username
          });
  
          if (response.status === 200) {
            this.frmMovimentoTable = this.frmMovimentoTable.map(row =>
              row.MV_CODIGO === this.selectedRow.MV_CODIGO ? this.selectedRow : row
            );
            this.isEditing = false;
          } else {
      ...
      printPDF() {
        ...
        doc.autoTable({
          ...
          body: this.frmMovimentoTable.map(row => this.columns.map(col => row[col.field])),
          theme: 'grid',
        });
