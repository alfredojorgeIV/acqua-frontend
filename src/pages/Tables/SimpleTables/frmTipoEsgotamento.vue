    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Tipo de Esgotamento</h1>
        </div>
        <div class="flex gap-4">
          <div class="w-61 margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmTipoEsgotamentoTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10"
              dataKey="TIPO_ESGOTAMENTO" @row-click="handleRowClick" :rowClass="rowClass">
          ...
          <div class="w-35">
            <div class="box-tables-info" v-if="selectedRow && !showAddBox">
              ...
              <input v-model="selectedRow.TIPO_ESGOTAMENTO" maxlength="5" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
              <p class="p-labels">Descrição</p>
              <input v-model="selectedRow.DESC_ESGOT" maxlength="15" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
            ...
            <div class="box-tables-info" v-if="showAddBox && !selectedRow">
              <p class="p-labels">Código</p>
              <input class="inputs margin-bottom-30" maxlength="5" v-model="addTipoEsgotamento" />
              <p class="p-labels">Descrição</p>
              <input class="inputs margin-bottom-30" maxlength="15" v-model="addDescEsgot" />
              <div class="flex justify-center gap-4 mt-40">
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        {
          name: "TIPO_ESGOTAMENTO",
          required: true,
          label: "Código",
          align: "left",
          field: "TIPO_ESGOTAMENTO",
        },
        {
          name: "DESC_ESGOT",
          required: true,
  ...
  methods: {
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.TIPO_ESGOTAMENTO === data.TIPO_ESGOTAMENTO
      };
    },
    ...
    async addToTable() {
      ...
      try {
        const response = await axios.post(`${baseUrl}/TIPO_ESGOTAMENTO_ADD`, {
          TIPO_ESGOTAMENTO: Number(this.addTipoEsgotamento),
          DESC_ESGOT: this.addDescEsgot,
    ...
    async fetchData() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.get(`${baseUrl}/TIPO_ESGOTAMENTO_ALL`);
        this.data = response.data;
        this.frmTipoEsgotamentoTable = this.data;
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
      } finally {
    ...
    async deleteFromTable() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.delete(`${baseUrl}/TIPO_ESGOTAMENTO_DELETE/${this.selectedRow.TIPO_ESGOTAMENTO}/${user.username}`);
        console.log(response.data);
        await this.fetchData();
    ...
    async saveUpdate() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.post(`${baseUrl}/TIPO_ESGOTAMENTO_UPDATE`, {
          TIPO_ESGOTAMENTO: Number(this.selectedRow.TIPO_ESGOTAMENTO),
          DESC_ESGOT: this.selectedRow.DESC_ESGOT,
          NOME_ATUALIZ: user.username
        });

        if (response.status === 200) {
          this.frmTipoEsgotamentoTable = this.frmTipoEsgotamentoTable.map(row =>
            row.TIPO_ESGOTAMENTO === this.selectedRow.TIPO_ESGOTAMENTO ? this.selectedRow : row
          );
          this.isEditing = false;
        } else {
          console.error("Erro ao atualizar o Tipo de Esgotamento");
        }
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmTipoEsgotamentoTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
      });
