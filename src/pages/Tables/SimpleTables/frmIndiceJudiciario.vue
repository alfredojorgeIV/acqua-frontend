    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Índice Judiciário</h1>
        </div>
        <div class="flex gap-4">
          <div class="w-61 margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmIndiceJudiciarioTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10"
              dataKey="DATA_INDICE" @row-click="handleRowClick" :rowClass="rowClass">
          ...
          <div class="w-35">
            <div class="box-tables-info" v-if="selectedRow && !showAddBox">
              ...
              <input type="date" v-model="selectedRow.DATA_INDICE" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
              <p class="p-labels">Descrição</p>
              <input v-model="selectedRow.INDICE" maxlength="15" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
            ...
            <div class="box-tables-info" v-if="showAddBox && !selectedRow">
              <p class="p-labels">Código</p>
              <input type="date" class="inputs margin-bottom-30" v-model="addDataIndice" />
              <p class="p-labels">Descrição</p>
              <input class="inputs margin-bottom-30" maxlength="15" v-model="addIndice" />
              <div class="flex justify-center gap-4 mt-40">
...
export default {
  ...
  methods: {
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.DATA_INDICE === data.DATA_INDICE
      };
    },
    ...
    async addToTable() {
      ...
      try {
        const response = await axios.post(`${baseUrl}/INDICE_JUDICIARIO_ADD`, {
          DATA_INDICE: new Date(this.addDataIndice).toISOString(), // Formatar a data como ISO-8601 completa
          INDICE: parseFloat(this.addIndice), // Converter para decimal
    ...
    async fetchData() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.get(`${baseUrl}/INDICE_JUDICIARIO_ALL`);
        this.data = response.data;
        this.frmIndiceJudiciarioTable = this.data.map(item => ({
          ...item,
          DATA_INDICE: new Date(item.DATA_INDICE).toISOString().split('T')[0] // Formatar para exibição
        }));
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
    ...
    async deleteFromTable() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.delete(`${baseUrl}/INDICE_JUDICIARIO_DELETE/${this.selectedRow.DATA_INDICE}/${user.username}`);
        console.log(response.data);
        await this.fetchData();
    ...
    async saveUpdate() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.post(`${baseUrl}/INDICE_JUDICIARIO_UPDATE`, {
          DATA_INDICE: new Date(this.selectedRow.DATA_INDICE).toISOString(), // Formatar a data como ISO-8601 completa
          INDICE: parseFloat(this.selectedRow.INDICE), // Converter para decimal
          NOME_ATUALIZ: user.username
        });

        if (response.status === 200) {
          this.frmIndiceJudiciarioTable = this.frmIndiceJudiciarioTable.map(row =>
            row.DATA_INDICE === this.selectedRow.DATA_INDICE ? this.selectedRow : row
          );
          this.isEditing = false;
        } else {
          console.error("Erro ao atualizar o INDICE_JUDICIARIO");
        }
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmIndiceJudiciarioTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
      });
