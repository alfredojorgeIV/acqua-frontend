    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Padrão do Imóvel</h1>
        </div>
        <div class="flex gap-4">
          <div class="w-61 margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmPadraoImovelTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10"
              dataKey="PADRAO_IMOVEL" @row-click="handleRowClick" :rowClass="rowClass">
          ...
          <div class="w-35">
            <div class="box-tables-info" v-if="selectedRow && !showAddBox">
              ...
              <input v-model="selectedRow.PADRAO_IMOVEL" maxlength="5" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
              <p class="p-labels">Descrição</p>
              <input v-model="selectedRow.DESC_PADRAO" maxlength="15" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
            ...
            <div class="box-tables-info" v-if="showAddBox && !selectedRow">
              <p class="p-labels">Código</p>
              <input class="inputs margin-bottom-30" maxlength="5" v-model="addPadraoImovel" />
              <p class="p-labels">Descrição</p>
              <input class="inputs margin-bottom-30" maxlength="15" v-model="addDescPadrao" />
              <div class="flex justify-center gap-4 mt-40">
...
export default {
  ...
  methods: {
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.PADRAO_IMOVEL === data.PADRAO_IMOVEL
      };
    },
    ...
    async addToTable() {
      ...
      try {
        const response = await axios.post(`${baseUrl}/PADRAO_IMOVEL_ADD`, {
          PADRAO_IMOVEL: Number(this.addPadraoImovel),
          DESC_PADRAO: this.addDescPadrao,
    ...
    async fetchData() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.get(`${baseUrl}/PADRAO_IMOVEL_ALL`);
        this.data = response.data;
        this.frmPadraoImovelTable = this.data;
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
      } finally {
    ...
    async deleteFromTable() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.delete(`${baseUrl}/PADRAO_IMOVEL_DELETE/${this.selectedRow.PADRAO_IMOVEL}/${user.username}`);
        console.log(response.data);
        await this.fetchData();
    ...
    async saveUpdate() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.post(`${baseUrl}/PADRAO_IMOVEL_UPDATE`, {
          PADRAO_IMOVEL: Number(this.selectedRow.PADRAO_IMOVEL),
          DESC_PADRAO: this.selectedRow.DESC_PADRAO,
          NOME_ATUALIZ: user.username
        });

        if (response.status === 200) {
          this.frmPadraoImovelTable = this.frmPadraoImovelTable.map(row =>
            row.PADRAO_IMOVEL === this.selectedRow.PADRAO_IMOVEL ? this.selectedRow : row
          );
          this.isEditing = false;
        } else {
          console.error("Erro ao atualizar o Padrão do Imóvel");
        }
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmPadraoImovelTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
      });
