    ...
    methods: {
      ...
      rowClass(data) {
        return {
          "pointer-cursor": true,
          "active-row":
            this.selectedRow && this.selectedRow.SISTEMA === data.SISTEMA,
        };
      },
      ...
      async addToTable() {
        ...
        try {
          const response = await axios.post(`${baseUrl}/SISTEMA_ADD`, {
            SISTEMA: this.addSistema,
            DESCRICAO: this.addDescricao,
            SEQUENCIA: Number(this.addSequencia),
      ...
      async fetchData() {
        ...
        const baseUrl = user.databaseUrl || "http://localhost:3000";
        try {
          const response = await axios.get(`${baseUrl}/SISTEMA_ALL`);
          this.data = response.data;
          this.frmSistemaTable = this.data;
        } catch (error) {
          console.error("Erro:", error);
          this.error = error.toString();
        } finally {
      ...
      async deleteFromTable() {
        ...
        try {
          const response = await axios.delete(
            `${baseUrl}/SISTEMA_DELETE/${this.selectedRow.SISTEMA}/${user.username}`
          );
          console.log(response.data);
          await this.fetchData();
      ...
      async saveUpdate() {
        ...
        const baseUrl = user.databaseUrl || "http://localhost:3000";
        try {
          const response = await axios.post(`${baseUrl}/SISTEMA_UPDATE`, {
            SISTEMA: this.selectedRow.SISTEMA,
            DESCRICAO: this.selectedRow.DESCRICAO,
            SEQUENCIA: Number(this.selectedRow.SEQUENCIA),
            NOME_ATUALIZ: user.username,
          });
  
          if (response.status === 200) {
            this.frmSistemaTable = this.frmSistemaTable.map((row) =>
              row.SISTEMA === this.selectedRow.SISTEMA
                ? this.selectedRow
                : row
            );
            this.isEditing = false;
      ...
      printPDF() {
        ...
        doc.autoTable({
          ...
          body: this.frmSistemaTable.map((row) =>
