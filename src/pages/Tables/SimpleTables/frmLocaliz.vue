    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Localizar Lote Quadra</h1>
        </div>
        <div class="flex gap-4">
          <div class="w-61 margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmLocalizTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10"
              dataKey="LOCALIZ_LOTE" @row-click="handleRowClick" :rowClass="rowClass">
          ...
          <div class="w-35">
            <div class="box-tables-info" v-if="selectedRow && !showAddBox">
              ...
              <input v-model="selectedRow.LOCALIZ_LOTE" maxlength="5" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
              <p class="p-labels">Descrição</p>
              <input v-model="selectedRow.DESCR_LOCALIZ_LOTE" maxlength="15" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
            ...
            <div class="box-tables-info" v-if="showAddBox && !selectedRow">
              <p class="p-labels">Código</p>
              <input class="inputs margin-bottom-30" maxlength="5" v-model="addLocalizLote" />
              <p class="p-labels">Descrição</p>
              <input class="inputs margin-bottom-30" maxlength="15" v-model="addDescrLocalizLote" />
              <div class="flex justify-center gap-4 mt-40">
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {
          name: "DESCR_LOCALIZ_LOTE",
          required: true,
          label: "Descrição",
          align: "left",
          field: "DESCR_LOCALIZ_LOTE",
        },
      ],
      frmLocalizTable: []
    };
  ...
  methods: {
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.LOCALIZ_LOTE === data.LOCALIZ_LOTE
      };
    },
    ...
    async addToTable() {
      ...
      try {
        const response = await axios.post(`${baseUrl}/LOCALIZ_LOTE_QUAD_ADD`, {
          LOCALIZ_LOTE: Number(this.addLocalizLote),
          DESCR_LOCALIZ_LOTE: this.addDescrLocalizLote,
    ...
    async fetchData() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.get(`${baseUrl}/LOCALIZ_LOTE_QUAD_ALL`);
        this.data = response.data;
        this.frmLocalizTable = this.data;
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
      } finally {
    ...
    async deleteFromTable() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.delete(`${baseUrl}/LOCALIZ_LOTE_QUAD_DELETE/${this.selectedRow.LOCALIZ_LOTE}/${user.username}`);
        console.log(response.data);
        await this.fetchData();
    ...
    async saveUpdate() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.post(`${baseUrl}/LOCALIZ_LOTE_QUAD_UPDATE`, {
          LOCALIZ_LOTE: Number(this.selectedRow.LOCALIZ_LOTE),
          DESCR_LOCALIZ_LOTE: this.selectedRow.DESCR_LOCALIZ_LOTE,
          NOME_ATUALIZ: user.username
        });

        if (response.status === 200) {
          this.frmLocalizTable = this.frmLocalizTable.map(row =>
            row.LOCALIZ_LOTE === this.selectedRow.LOCALIZ_LOTE ? this.selectedRow : row
          );
          this.isEditing = false;
        } else {
          console.error("Erro ao atualizar a Localiz Lote Quadra");
        }
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmLocalizTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
      });
