    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Setor de Abastecimento</h1>
        </div>
        <div class="flex gap-4">
          <div class="w-61 margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmSetorAbastecTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10"
              dataKey="SA_CODIGO" @row-click="handleRowClick" :rowClass="rowClass">
          ...
          <div class="w-35">
            <div class="box-tables-info" v-if="selectedRow && !showAddBox">
              ...
              <input v-model="selectedRow.SA_CODIGO" maxlength="5" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
              <p class="p-labels">Descrição</p>
              <input v-model="selectedRow.SA_DESCRICAO" maxlength="15" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
            ...
            <div class="box-tables-info" v-if="showAddBox && !selectedRow">
              <p class="p-labels">Código</p>
              <input class="inputs margin-bottom-30" maxlength="5" v-model="addSaCodigo" />
              <p class="p-labels">Descrição</p>
              <input class="inputs margin-bottom-30" maxlength="15" v-model="addSadescricao" />
              <div class="flex justify-center gap-4 mt-40">
...
export default {
  ...
  methods: {
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.SA_CODIGO === data.SA_CODIGO
      };
    },
    ...
    async addToTable() {
      ...
      try {
        const response = await axios.post(`${baseUrl}/SETOR_ABASTECIMENTO_ADD`, {
          SA_CODIGO: Number(this.addSaCodigo),
          SA_DESCRICAO: this.addSadescricao,
    ...
    async fetchData() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.get(`${baseUrl}/SETOR_ABASTECIMENTO_ALL`);
        this.data = response.data;
        this.frmSetorAbastecTable = this.data;
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
      } finally {
    ...
    async deleteFromTable() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.delete(`${baseUrl}/SETOR_ABASTECIMENTO_DELETE/${this.selectedRow.SA_CODIGO}/${user.username}`);
        console.log(response.data);
        await this.fetchData();
    ...
    async saveUpdate() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.post(`${baseUrl}/SETOR_ABASTECIMENTO_UPDATE`, {
          SA_CODIGO: Number(this.selectedRow.SA_CODIGO),
          SA_DESCRICAO: this.selectedRow.SA_DESCRICAO,
          NOME_ATUALIZ: user.username
        });

        if (response.status === 200) {
          this.frmSetorAbastecTable = this.frmSetorAbastecTable.map(row =>
            row.SA_CODIGO === this.selectedRow.SA_CODIGO ? this.selectedRow : row
          );
          this.isEditing = false;
        } else {
          console.error("Erro ao atualizar o Setor de Abastecimento");
        }
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmSetorAbastecTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
      });
