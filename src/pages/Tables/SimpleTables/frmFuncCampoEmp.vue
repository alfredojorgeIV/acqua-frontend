      ...
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">
            Funcionário de Campo da Empreiteira
          </h1>
        </div>
        <div class="flex gap-4">
          ...
          <div class="w-35">
            <div class="box-tables-info" v-if="selectedRow && !showAddBox">
              ...
              <input
                v-model="selectedRow.CODIGO_FUNC"
                maxlength="5"
  ...
  methods: {
    ...
    rowClass(data) {
      return {
        "pointer-cursor": true,
        "active-row":
          this.selectedRow && this.selectedRow.CODIGO_FUNC === data.CODIGO_FUNC,
      };
    },
    ...
    async addToTable() {
      ...
      try {
        const response = await axios.post(`${baseUrl}/FUNCIONARIO_CAMPO_EMPREITEIRA_ADD`, {
          CODIGO_FUNC: Number(this.addCodigoFunc),
          NOME_FUNC: this.addNomeFunc,
          CODIGO: this.addCodigo,
    ...
    async fetchData() {
      ...
      const baseUrl = user.databaseUrl || "http://localhost:3000";
      try {
        const response = await axios.get(`${baseUrl}/FUNCIONARIO_CAMPO_EMPREITEIRA_ALL`);
        this.data = response.data;
        this.frmFuncCampoEmpTable = this.data;
      } catch (error) {
        console.error("Erro:", error);
        this.error = error.toString();
      } finally {
    ...
    async deleteFromTable() {
      ...
      try {
        const response = await axios.delete(
          `${baseUrl}/FUNCIONARIO_CAMPO_EMPREITEIRA_DELETE/${this.selectedRow.CODIGO_FUNC}/${user.username}`
        );
        console.log(response.data);
        await this.fetchData();
    ...
    async saveUpdate() {
      ...
      const baseUrl = user.databaseUrl || "http://localhost:3000";
      try {
        const response = await axios.post(`${baseUrl}/FUNCIONARIO_CAMPO_EMPREITEIRA_UPDATE`, {
        ...
        if (response.status === 200) {
          this.frmFuncCampoEmpTable = this.frmFuncCampoEmpTable.map((row) =>
            row.CODIGO_FUNC === this.selectedRow.CODIGO_FUNC
              ? this.selectedRow
              : row
          );
          this.isEditing = false;
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmFuncCampoEmpTable.map((row) =>
