    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Material do Cavalete</h1>
        </div>
        <div class="flex gap-4">
          <div class="w-61 margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmCavaleteTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10"
              dataKey="MAT_CAVALETE" @row-click="handleRowClick" :rowClass="rowClass">
          ...
          <div class="w-35">
            <div class="box-tables-info" v-if="selectedRow && !showAddBox">
              ...
              <input v-model="selectedRow.MAT_CAVALETE" maxlength="5" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
              <p class="p-labels">Descrição</p>
              <input v-model="selectedRow.DESC_MAT_CAVALETE" maxlength="15" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
            ...
            <div class="box-tables-info" v-if="showAddBox && !selectedRow">
              <p class="p-labels">Código</p>
              <input class="inputs margin-bottom-30" maxlength="5" v-model="addMatCavalete" />
              <p class="p-labels">Descrição</p>
              <input class="inputs margin-bottom-30" maxlength="15" v-model="addDescMatCavalete" />
              <div class="flex justify-center gap-4 mt-40">
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {
          name: "DESC_MAT_CAVALETE",
          required: true,
          label: "Descrição",
          align: "left",
          field: "DESC_MAT_CAVALETE",
        },
      ],
      frmCavaleteTable: []
    };
  ...
  methods: {
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.MAT_CAVALETE === data.MAT_CAVALETE
      };
    },
    ...
    async addToTable() {
      ...
      try {
        const response = await axios.post(`${baseUrl}/MAT_CAVALETE_ADD`, {
          MAT_CAVALETE: Number(this.addMatCavalete),
          DESC_MAT_CAVALETE: this.addDescMatCavalete,
    ...
    async fetchData() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.get(`${baseUrl}/MAT_CAVALETE_ALL`);
        this.data = response.data;
        this.frmCavaleteTable = this.data;
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
      } finally {
    ...
    async deleteFromTable() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.delete(`${baseUrl}/MAT_CAVALETE_DELETE/${this.selectedRow.MAT_CAVALETE}/${user.username}`);
        console.log(response.data);
        await this.fetchData();
    ...
    async saveUpdate() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.post(`${baseUrl}/MAT_CAVALETE_UPDATE`, {
          MAT_CAVALETE: Number(this.selectedRow.MAT_CAVALETE),
          DESC_MAT_CAVALETE: this.selectedRow.DESC_MAT_CAVALETE,
          NOME_ATUALIZ: user.username
        });

        if (response.status === 200) {
          this.frmCavaleteTable = this.frmCavaleteTable.map(row =>
            row.MAT_CAVALETE === this.selectedRow.MAT_CAVALETE ? this.selectedRow : row
          );
          this.isEditing = false;
        } else {
          console.error("Erro ao atualizar o Material do Cavalete");
        }
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmCavaleteTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
      });
