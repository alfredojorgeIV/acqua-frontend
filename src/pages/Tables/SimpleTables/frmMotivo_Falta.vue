    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Motivo da Falta</h1>
        </div>
        <div class="flex gap-4">
          <div class="w-61 margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmMotivo_FaltaTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10"
...
export default {
  ...
  methods: {
    ...
    async addToTable() {
      ...
      try {
        const response = await axios.post(`${baseUrl}/MOTIVO_FALTA_ADD`, {
    ...
    async fetchData() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.get(`${baseUrl}/MOTIVO_FALTA_ALL`);
        this.data = response.data;
        this.frmMotivo_FaltaTable = this.data;
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
      } finally {
    ...
    async deleteFromTable() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.delete(`${baseUrl}/MOTIVO_FALTA_DELETE/${this.selectedRow.CODIGO}/${user.username}`);
        console.log(response.data);
        await this.fetchData();
    ...
    async saveUpdate() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.post(`${baseUrl}/MOTIVO_FALTA_UPDATE`, {
        ...
        if (response.status === 200) {
          this.frmMotivo_FaltaTable = this.frmMotivo_FaltaTable.map(row =>
            row.CODIGO === this.selectedRow.CODIGO ? this.selectedRow : row
          );
          this.isEditing = false;
        } else {
          console.error("Erro ao atualizar o Motivo da Falta");
        }
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmMotivo_FaltaTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
      });
