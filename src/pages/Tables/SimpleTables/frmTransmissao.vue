    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Transmissão do HD</h1>
        </div>
        <div class="flex gap-4">
          <div class="w-61 margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmTransmissaoTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10"
              dataKey="TRANSMISSAO_HD" @row-click="handleRowClick" :rowClass="rowClass">
          ...
          <div class="w-35">
            <div class="box-tables-info" v-if="selectedRow && !showAddBox">
              ...
              <input v-model="selectedRow.TRANSMISSAO_HD" maxlength="5" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
              <p class="p-labels">Descrição</p>
              <input v-model="selectedRow.DESC_TRANSMIS_HD" maxlength="15" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
            ...
            <div class="box-tables-info" v-if="showAddBox && !selectedRow">
              <p class="p-labels">Código</p>
              <input class="inputs margin-bottom-30" maxlength="5" v-model="addTransmissaoHd" />
              <p class="p-labels">Descrição</p>
              <input class="inputs margin-bottom-30" maxlength="15" v-model="addDescTransmisHd" />
              <div class="flex justify-center gap-4 mt-40">
...
export default {
      ...
      columns: [
        {
          ...
          label: "Código",
          align: "left",
          field: "TRANSMISSAO_HD",
        },
        {
          name: "DESC_TRANSMIS_HD",
          required: true,
          label: "Descrição",
          align: "left",
          field: "DESC_TRANSMIS_HD",
        },
      ],
      frmTransmissaoTable: []
    };
  ...
  methods: {
    handleAddRowClick(event) {
      ...
      this.addTransmissaoHd = '';
      this.addDescTransmisHd = '';
    },
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.TRANSMISSAO_HD === data.TRANSMISSAO_HD
      };
    },
    ...
    async addToTable() {
      ...
      try {
        const response = await axios.post(`${baseUrl}/TRANSMISSAO_HD_ADD`, {
          TRANSMISSAO_HD: Number(this.addTransmissaoHd),
          DESC_TRANSMIS_HD: this.addDescTransmisHd,
    ...
    async fetchData() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.get(`${baseUrl}/TRANSMISSAO_HD_ALL`);
        this.data = response.data;
        this.frmTransmissaoTable = this.data;
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
      } finally {
    ...
    async deleteFromTable() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.delete(`${baseUrl}/TRANSMISSAO_HD_DELETE/${this.selectedRow.TRANSMISSAO_HD}/${user.username}`);
        console.log(response.data);
        await this.fetchData();
    ...
    async saveUpdate() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.post(`${baseUrl}/TRANSMISSAO_HD_UPDATE`, {
          TRANSMISSAO_HD: Number(this.selectedRow.TRANSMISSAO_HD),
          DESC_TRANSMIS_HD: this.selectedRow.DESC_TRANSMIS_HD,
          NOME_ATUALIZ: user.username
        });

        if (response.status === 200) {
          this.frmTransmissaoTable = this.frmTransmissaoTable.map(row =>
            row.TRANSMISSAO_HD === this.selectedRow.TRANSMISSAO_HD ? this.selectedRow : row
          );
          this.isEditing = false;
        } else {
          console.error("Erro ao atualizar a Transmissão");
        }
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmTransmissaoTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
      });
