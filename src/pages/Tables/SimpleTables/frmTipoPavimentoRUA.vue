      ...
      <div class="content">
        <section>
          <div class="intro-services-to-be-billed">
            <h1 ref="title" class="title-page">Tipo do Pavimento da Rua</h1>
          </div>
          <div class="flex gap-4">
            <div class="w-61 margin-top-30">
              ...
              <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
                :value="frmTipoPavimentoRuaTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10"
  ...
  export default {
    ...
    methods: {
      ...
      async addToTable() {
        ...
        try {
          const response = await axios.post(`${baseUrl}/TIPO_PAVIMENTO_RUA_ADD`, {
      ...
      async fetchData() {
        ...
        const baseUrl = user.databaseUrl || 'http://localhost:3000';
        try {
          const response = await axios.get(`${baseUrl}/TIPO_PAVIMENTO_RUA_ALL`);
          this.data = response.data;
          this.frmTipoPavimentoRuaTable = this.data;
        } catch (error) {
          console.error('Erro:', error);
          this.error = error.toString();
        } finally {
      ...
      async deleteFromTable() {
        ...
        const baseUrl = user.databaseUrl || 'http://localhost:3000';
        try {
          const response = await axios.delete(`${baseUrl}/TIPO_PAVIMENTO_RUA_DELETE/${this.selectedRow.TR_CODIGO}/${user.username}`);
          console.log(response.data);
          await this.fetchData();
      ...
      async saveUpdate() {
        ...
        const baseUrl = user.databaseUrl || 'http://localhost:3000';
        try {
          const response = await axios.post(`${baseUrl}/TIPO_PAVIMENTO_RUA_UPDATE`, {
          ...
          if (response.status === 200) {
            this.frmTipoPavimentoRuaTable = this.frmTipoPavimentoRuaTable.map(row =>
      ...
      printPDF() {
        ...
        doc.autoTable({
          ...
          body: this.frmTipoPavimentoRuaTable.map(row => this.columns.map(col => row[col.field])),
          theme: 'grid',
        });
