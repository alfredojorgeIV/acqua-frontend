    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 class="title-page">Diâmetro do HD</h1>
        </div>
        <div class="flex gap-4">
          <div class="w-61 margin-top-30">
            ...
            <DataTable style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="hdDiameterTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="id"
          ...
          <div class="w-35">
            <div class="box-tables-info" v-if="selectedRow && !showAddBox">
              ...
              <input v-model="selectedRow.DIAMETRO_HD" class="inputs margin-bottom-30" :disabled="!isEditing" />
              <p class="p-labels">Descrição</p>
              <input v-model="selectedRow.DESC_DIAMETRO_HD" class="inputs margin-bottom-30" :disabled="!isEditing" />
            ...
            <div class="box-tables-info" v-if="showAddBox && !selectedRow">
              ...
              <div class="flex justify-center gap-4 mt-40">
                <Button class="primary-button flex align-center" @click="addDIAMETRO_HDTarifa">
...
export default {
  ...
  methods: {
    ...
    async addDIAMETRO_HDTarifa() {
      this.isLoading = true;
      try {
        const response = await axios.post('http://localhost:3000/DIAMETRO_HD_ADD', {
          DIAMETRO_HD: Number(this.addDIAMETRO_HD),
          DESC_DIAMETRO_HD: this.addDescricao,
          NOME_ATUALIZ: "Luiza"
        });
    ...
    async fetchData() {
      this.isLoading = true;
      try {
        const response = await axios.get('http://localhost:3000/DIAMETRO_HD_ALL');
        this.data = response.data;
        this.hdDiameterTable = this.data;
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
      } finally {
    ...
    async deleteTaxCategory() {
      this.isLoading = true;
      try {
        const response = await axios.delete(`http://localhost:3000/DIAMETRO_HD_DELETE/${this.selectedRow.DIAMETRO_HD}`);
        console.log(response.data);
        await this.fetchData();
    ...
    async saveUpdate() {
      this.isLoading = true;
      try {
        const response = await axios.post(`http://localhost:3000/DIAMETRO_HD_UPDATE`, {
          DIAMETRO_HD: this.selectedRow.DIAMETRO_HD,
        ...
        if (response.status === 200) {
          this.hdDiameterTable = this.hdDiameterTable.map(row => row.DIAMETRO_HD === this.selectedRow.DIAMETRO_HD ? this.selectedRow : row);
          this.isEditing = false;
        } else {
          console.error("Erro ao atualizar a DIAMETRO_HD de tarifa");
        }
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
