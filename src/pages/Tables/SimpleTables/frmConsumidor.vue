    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Tipo de Consumidor</h1>
        </div>
        <div class="flex gap-4">
          <div class="w-61 margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmConsumidorTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10"
              dataKey="TIPO_CONSUMIDOR" @row-click="handleRowClick" :rowClass="rowClass">
          ...
          <div class="w-35">
            <div class="box-tables-info" v-if="selectedRow && !showAddBox">
              ...
              <input v-model="selectedRow.TIPO_CONSUMIDOR" maxlength="5" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
              <p class="p-labels">Descrição</p>
              <input v-model="selectedRow.DESC_CONSUMIDOR" maxlength="15" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
            ...
            <div class="box-tables-info" v-if="showAddBox && !selectedRow">
              <p class="p-labels">Código</p>
              <input class="inputs margin-bottom-30" maxlength="5" v-model="addTipoConsumidor" />
              <p class="p-labels">Descrição</p>
              <input class="inputs margin-bottom-30" maxlength="15" v-model="addDescConsumidor" />
              <div class="flex justify-center gap-4 mt-40">
...
export default {
      ...
      columns: [
        {
          ...
          label: "Código",
          align: "left",
          field: "TIPO_CONSUMIDOR",
        },
        {
          name: "DESC_CONSUMIDOR",
          required: true,
          label: "Descrição",
          align: "left",
          field: "DESC_CONSUMIDOR",
        },
      ],
      frmConsumidorTable: []
    };
  ...
  methods: {
    handleAddRowClick(event) {
      ...
      this.addTipoConsumidor = '';
      this.addDescConsumidor = '';
    },
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.TIPO_CONSUMIDOR === data.TIPO_CONSUMIDOR
      };
    },
    ...
    async addToTable() {
      ...
      try {
        const response = await axios.post(`${baseUrl}/TIPO_CONSUMIDOR_ADD`, {
          TIPO_CONSUMIDOR: Number(this.addTipoConsumidor),
          DESC_CONSUMIDOR: this.addDescConsumidor,
    ...
    async fetchData() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.get(`${baseUrl}/TIPO_CONSUMIDOR_ALL`);
        this.data = response.data;
        this.frmConsumidorTable = this.data;
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
      } finally {
    ...
    async deleteFromTable() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.delete(`${baseUrl}/TIPO_CONSUMIDOR_DELETE/${this.selectedRow.TIPO_CONSUMIDOR}/${user.username}`);
        console.log(response.data);
        await this.fetchData();
    ...
    async saveUpdate() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.post(`${baseUrl}/TIPO_CONSUMIDOR_UPDATE`, {
          TIPO_CONSUMIDOR: Number(this.selectedRow.TIPO_CONSUMIDOR),
          DESC_CONSUMIDOR: this.selectedRow.DESC_CONSUMIDOR,
          NOME_ATUALIZ: user.username
        });

        if (response.status === 200) {
          this.frmConsumidorTable = this.frmConsumidorTable.map(row =>
            row.TIPO_CONSUMIDOR === this.selectedRow.TIPO_CONSUMIDOR ? this.selectedRow : row
          );
          this.isEditing = false;
        } else {
          console.error("Erro ao atualizar o Tipo de Consumidor");
        }
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmConsumidorTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
      });
