    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Prorrogação de Ordem de Serviço</h1>
        </div>
        <div class="flex gap-4">
          <div class="w-61 margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmProrrogOsTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10"
              dataKey="MT_PRORROG_OS" @row-click="handleRowClick" :rowClass="rowClass">
          ...
          <div class="w-35">
            <div class="box-tables-info" v-if="selectedRow && !showAddBox">
              ...
              <input v-model="selectedRow.MT_PRORROG_OS" maxlength="5" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
              <p class="p-labels">Descrição</p>
              <input v-model="selectedRow.DESC_PRORROG_OS" maxlength="15" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
            ...
            <div class="box-tables-info" v-if="showAddBox && !selectedRow">
              <p class="p-labels">Código</p>
              <input class="inputs margin-bottom-30" maxlength="5" v-model="addMtProrrogOs" />
              <p class="p-labels">Descrição</p>
              <input class="inputs margin-bottom-30" maxlength="15" v-model="addDescProrrogOs" />
              <div class="flex justify-center gap-4 mt-40">
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {
          name: "DESC_PRORROG_OS",
          required: true,
          label: "Descrição",
          align: "left",
          field: "DESC_PRORROG_OS",
        },
      ],
      frmProrrogOsTable: []
    };
  ...
  methods: {
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.MT_PRORROG_OS === data.MT_PRORROG_OS
      };
    },
    ...
    async addToTable() {
      ...
      try {
        const response = await axios.post(`${baseUrl}/MOTIVO_PRORROG_OS_ADD`, {
          MT_PRORROG_OS: Number(this.addMtProrrogOs),
          DESC_PRORROG_OS: this.addDescProrrogOs,
    ...
    async fetchData() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.get(`${baseUrl}/MOTIVO_PRORROG_OS_ALL`);
        this.data = response.data;
        this.frmProrrogOsTable = this.data;
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
      } finally {
    ...
    async deleteFromTable() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.delete(`${baseUrl}/MOTIVO_PRORROG_OS_DELETE/${this.selectedRow.MT_PRORROG_OS}/${user.username}`);
        console.log(response.data);
        await this.fetchData();
    ...
    async saveUpdate() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.post(`${baseUrl}/MOTIVO_PRORROG_OS_UPDATE`, {
          MT_PRORROG_OS: Number(this.selectedRow.MT_PRORROG_OS),
          DESC_PRORROG_OS: this.selectedRow.DESC_PRORROG_OS,
          NOME_ATUALIZ: user.username
        });

        if (response.status === 200) {
          this.frmProrrogOsTable = this.frmProrrogOsTable.map(row =>
            row.MT_PRORROG_OS === this.selectedRow.MT_PRORROG_OS ? this.selectedRow : row
          );
          this.isEditing = false;
        } else {
          console.error("Erro ao atualizar o Prorrogação da Ordem de Serviço");
        }
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmProrrogOsTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
      });
