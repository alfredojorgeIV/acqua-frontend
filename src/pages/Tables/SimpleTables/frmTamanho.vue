    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Tamanho do HD</h1>
        </div>
        <div class="flex gap-4">
          <div class="w-61 margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmTamanhoTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10"
              dataKey="TAMANHO_HD" @row-click="handleRowClick" :rowClass="rowClass">
          ...
          <div class="w-35">
            <div class="box-tables-info" v-if="selectedRow && !showAddBox">
              ...
              <input v-model="selectedRow.TAMANHO_HD" maxlength="5" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
              <p class="p-labels">Descrição</p>
              <input v-model="selectedRow.DESC_TAMANHO_HD" maxlength="15" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
            ...
            <div class="box-tables-info" v-if="showAddBox && !selectedRow">
              <p class="p-labels">Código</p>
              <input class="inputs margin-bottom-30" maxlength="5" v-model="addTamanhoHd" />
              <p class="p-labels">Descrição</p>
              <input class="inputs margin-bottom-30" maxlength="15" v-model="addDescTamanhoHd" />
              <div class="flex justify-center gap-4 mt-40">
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {
          name: "DESC_TAMANHO_HD",
          required: true,
          label: "Descrição",
          align: "left",
          field: "DESC_TAMANHO_HD",
        },
      ],
      frmTamanhoTable: []
    };
  ...
  methods: {
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.TAMANHO_HD === data.TAMANHO_HD
      };
    },
    ...
    async addToTable() {
      ...
      try {
        const response = await axios.post(`${baseUrl}/TAMANHO_HD_ADD`, {
          TAMANHO_HD: Number(this.addTamanhoHd),
          DESC_TAMANHO_HD: this.addDescTamanhoHd,
    ...
    async fetchData() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.get(`${baseUrl}/TAMANHO_HD_ALL`);
        this.data = response.data;
        this.frmTamanhoTable = this.data;
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
      } finally {
    ...
    async deleteFromTable() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.delete(`${baseUrl}/TAMANHO_HD_DELETE/${this.selectedRow.TAMANHO_HD}/${user.username}`);
        console.log(response.data);
        await this.fetchData();
    ...
    async saveUpdate() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.post(`${baseUrl}/TAMANHO_HD_UPDATE`, {
          TAMANHO_HD: Number(this.selectedRow.TAMANHO_HD),
          DESC_TAMANHO_HD: this.selectedRow.DESC_TAMANHO_HD,
          NOME_ATUALIZ: user.username
        });

        if (response.status === 200) {
          this.frmTamanhoTable = this.frmTamanhoTable.map(row =>
            row.TAMANHO_HD === this.selectedRow.TAMANHO_HD ? this.selectedRow : row
          );
          this.isEditing = false;
        } else {
          console.error("Erro ao atualizar o Tamanho do HD");
        }
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmTamanhoTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
      });
