    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 class="title-page">Testemunha</h1>
        </div>
        <div class="flex gap-4">
          <div class="w-61 margin-top-30">
            ...
            <DataTable style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmTestemunhaTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="CHAVE"
          ...
          <div class="w-35">
            <div class="box-tables-info" v-if="selectedRow && !showAddBox">
              ...
              <input v-model="selectedRow.CHAVE" class="inputs margin-bottom-30" :disabled="!isEditing" />
              <p class="p-labels">Descrição</p>
              <input v-model="selectedRow.NOME" class="inputs margin-bottom-30" :disabled="!isEditing" />
              <p class="p-labels">Documento</p>
              <input v-model="selectedRow.DOCUMENTO" class="inputs margin-bottom-30" :disabled="!isEditing" />
            ...
            <div class="box-tables-info" v-if="showAddBox && !selectedRow">
              <p class="p-labels">Código</p>
              <input class="inputs margin-bottom-30" v-model="addChave" />
              <p class="p-labels">Nome</p>
              <input class="inputs margin-bottom-30" v-model="addNome" />
              <p class="p-labels">Documento</p>
              <input class="inputs margin-bottom-30" v-model="addDocumento" />
              <div class="flex justify-center gap-4 mt-40">
...
export default {
  ...
  methods: {
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.CHAVE === data.CHAVE
      };
    },
    ...
    async addToTable() {
      this.isLoading = true;
      try {
        const response = await axios.post('http://localhost:3000/TESTEMUNHA_EMPRESA_ADD', {
          CHAVE: this.addChave,
          NOME: this.addNome,
          DOCUMENTO: this.addDocumento,
          NOME_ATUALIZ: "Luiza"
        });
    ...
    async fetchData() {
      this.isLoading = true;
      try {
        const response = await axios.get('http://localhost:3000/TESTEMUNHA_EMPRESA_ALL');
        this.data = response.data;
        this.frmTestemunhaTable = this.data;
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
      } finally {
    ...
    async deleteFromTable() {
      this.isLoading = true;
      try {
        const response = await axios.delete(`http://localhost:3000/TESTEMUNHA_EMPRESA_DELETE/${this.selectedRow.CHAVE}`);
        console.log(response.data);
        await this.fetchData();
    ...
    async saveUpdate() {
      ...
      this.isEditing = false;
      try {
        const response = await axios.post(`http://localhost:3000/TESTEMUNHA_EMPRESA_UPDATE`, {
          CHAVE: this.selectedRow.TESTEMUNHA_EMPRESA,
          NOME: this.selectedRow.NOME,
          DOCUMENTO: this.selectedRow.DOCUMENTO,
          NOME_ATUALIZ: "Luiza"
        });

        if (response.status === 200) {
          this.frmTestemunhaTable = this.frmTestemunhaTable.map(row => row.CHAVE === this.selectedRow.CHAVE ? this.selectedRow : row);
          this.isEditing = false;
        } else {
