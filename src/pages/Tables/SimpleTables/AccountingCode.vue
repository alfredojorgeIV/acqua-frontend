    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 class="title-page">Código contábil</h1>
        </div>
        <div class="flex gap-4">
          <div class="w-61 margin-top-30">
            ...
            <DataTable style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="taxCategoryTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="id"
          ...
          <div class="w-35">
            <div class="box-tables-info" v-if="selectedRow && !showAddBox">
              ...
              <input v-model="selectedRow.COD_CONTA" class="inputs margin-bottom-30" :disabled="!isEditing" />
              <p class="p-labels">Descrição</p>
              <input v-model="selectedRow.DESC_CONTA" class="inputs margin-bottom-30" :disabled="!isEditing" />
              <p class="p-labels">Sequência </p>
              <input v-model="selectedRow.SEQUENCIA" class="inputs margin-bottom-30" :disabled="!isEditing" />
              <p class="p-labels">Descrição Crédito</p>
              <input v-model="selectedRow.CREDITO" class="inputs margin-bottom-30" :disabled="!isEditing" />
            ...
            <div class="box-tables-info" v-if="showAddBox && !selectedRow">
              ...
              <p class="p-labels">Sequencia</p>
              <input class="inputs margin-bottom-30" v-model="addSequencia" />
              <p class="p-labels">Crédito</p>
              <input class="inputs margin-bottom-30" v-model="addCredito" />
              <div class="flex justify-center gap-4 mt-40">
  ...
  methods: {
    ...
    async addCategoriaTarifa() {
      ...
      try {
        const response = await axios.post('http://localhost:3000/CATEGORIA_TARIFA_ADD', {
          COD_CONTA: Number(this.addCodigo),
          DESC_CONTA: this.addDescricao,
          SEQUENCIA: this.addSequencia,
          CREDITO: this.addCodigo,
          NOME_ATUALIZ: "Luiza"
        });
    ...
    async fetchData() {
      ...
      try {
        ...
        this.taxCategoryTable = this.data;
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
      } finally {
    ...
    async deleteTaxCategory() {
      this.isLoading = true;
      try {
        const response = await axios.delete(`http://localhost:3000/CATEGORIA_TARIFA_DELETE/${this.selectedRow.COD_CONTA}`);
        console.log(response.data);
        await this.fetchData();
    ...
    async saveUpdate() {
      ...
      try {
        ...
        if (response.status === 200) {
          this.taxCategoryTable = this.taxCategoryTable.map(row => row.COD_CONTA === this.selectedRow.COD_CONTA ? this.selectedRow : row);
          this.isEditing = false;
        } else {
          console.error("Erro ao atualizar a COD_CONTA de tarifa");
        }
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
