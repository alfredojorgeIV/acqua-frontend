    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Capacidade do Hd</h1>
        </div>
        <div class="flex gap-4">
          <div class="w-61 margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmCapacidadeTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10"
              dataKey="CAPAC_HD" @row-click="handleRowClick" :rowClass="rowClass">
          ...
          <div class="w-35">
            <div class="box-tables-info" v-if="selectedRow && !showAddBox">
              ...
              <input v-model="selectedRow.CAPAC_HD" maxlength="5" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
              <p class="p-labels">Descrição</p>
              <input v-model="selectedRow.DESC_CAPAC_HD" maxlength="15" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
            ...
            <div class="box-tables-info" v-if="showAddBox && !selectedRow">
              <p class="p-labels">Código</p>
              <input class="inputs margin-bottom-30" maxlength="5" v-model="addCapacidade" />
              <p class="p-labels">Descrição</p>
              <input class="inputs margin-bottom-30" maxlength="15" v-model="addDescCapacHd" />
              <div class="flex justify-center gap-4 mt-40">
...
export default {
  ...
  methods: {
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.CAPAC_HD === data.CAPAC_HD
      };
    },
    ...
    async addToTable() {
      ...
      try {
        const response = await axios.post(`${baseUrl}/CAPACIDADE_HD_ADD`, {
          CAPAC_HD: Number(this.addCapacidade),
    ...
    async fetchData() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.get(`${baseUrl}/CAPACIDADE_HD_ALL`);
        this.data = response.data;
        this.frmCapacidadeTable = this.data;
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
      } finally {
    ...
    async deleteFromTable() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.delete(`${baseUrl}/CAPACIDADE_HD_DELETE/${this.selectedRow.CAPAC_HD}/${user.username}`);
        console.log(response.data);
        await this.fetchData();
    ...
    async saveUpdate() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.post(`${baseUrl}/CAPACIDADE_HD_UPDATE`, {
        ...
        if (response.status === 200) {
          this.frmCapacidadeTable = this.frmCapacidadeTable.map(row =>
            row.CAPAC_HD === this.selectedRow.CAPAC_HD ? this.selectedRow : row
          );
          this.isEditing = false;
        } else {
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmCapacidadeTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
      });
