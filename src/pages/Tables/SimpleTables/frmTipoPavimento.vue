      ...
      <div class="content">
        <section>
          <div class="intro-services-to-be-billed">
            <h1 ref="title" class="title-page">Tipo de Pavimento</h1>
          </div>
          <div class="flex gap-4">
            <div class="w-61 margin-top-30">
              ...
              <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
                :value="frmTipoPavimentoTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10"
                dataKey="TP_CODIGO" @row-click="handleRowClick" :rowClass="rowClass">
            ...
            <div class="w-35">
              <div class="box-tables-info" v-if="selectedRow && !showAddBox">
                ...
                <input v-model="selectedRow.TP_CODIGO" maxlength="5" class="inputs margin-bottom-30"
                  :disabled="!isEditing" />
                <p class="p-labels">Descrição</p>
                <input v-model="selectedRow.TP_DESCRICAO" maxlength="15" class="inputs margin-bottom-30"
                  :disabled="!isEditing" />
              ...
              <div class="box-tables-info" v-if="showAddBox && !selectedRow">
                <p class="p-labels">Código</p>
                <input class="inputs margin-bottom-30" maxlength="5" v-model="addTpCodigo" />
                <p class="p-labels">Descrição</p>
                <input class="inputs margin-bottom-30" maxlength="15" v-model="addTpDescricao" />
                <div class="flex justify-center gap-4 mt-40">
  ...
  export default {
    ...
    methods: {
      ...
      rowClass(data) {
        return {
          'pointer-cursor': true,
          'active-row': this.selectedRow && this.selectedRow.TP_CODIGO === data.TP_CODIGO
        };
      },
      ...
      async addToTable() {
        ...
        try {
          const response = await axios.post(`${baseUrl}/TIPO_PAVIMENTO_ADD`, {
            TP_CODIGO: Number(this.addTpCodigo),
            TP_DESCRICAO: this.addTpDescricao,
      ...
      async fetchData() {
        ...
        const baseUrl = user.databaseUrl || 'http://localhost:3000';
        try {
          const response = await axios.get(`${baseUrl}/TIPO_PAVIMENTO_ALL`);
          this.data = response.data;
          this.frmTipoPavimentoTable = this.data;
        } catch (error) {
          console.error('Erro:', error);
          this.error = error.toString();
        } finally {
      ...
      async deleteFromTable() {
        ...
        const baseUrl = user.databaseUrl || 'http://localhost:3000';
        try {
          const response = await axios.delete(`${baseUrl}/TIPO_PAVIMENTO_DELETE/${this.selectedRow.TP_CODIGO}/${user.username}`);
          console.log(response.data);
          await this.fetchData();
      ...
      async saveUpdate() {
        ...
        const baseUrl = user.databaseUrl || 'http://localhost:3000';
        try {
          const response = await axios.post(`${baseUrl}/TIPO_PAVIMENTO_UPDATE`, {
            TP_CODIGO: Number(this.selectedRow.TP_CODIGO),
            TP_DESCRICAO: this.selectedRow.TP_DESCRICAO,
            NOME_ATUALIZ: user.username
          });
  
          if (response.status === 200) {
            this.frmTipoPavimentoTable = this.frmTipoPavimentoTable.map(row =>
              row.TP_CODIGO === this.selectedRow.TP_CODIGO ? this.selectedRow : row
            );
            this.isEditing = false;
          } else {
      ...
      printPDF() {
        ...
        doc.autoTable({
          ...
          body: this.frmTipoPavimentoTable.map(row => this.columns.map(col => row[col.field])),
          theme: 'grid',
        });
