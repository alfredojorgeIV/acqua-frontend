    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Tipo de Logradouro</h1>
        </div>
        <div class="flex gap-4">
          <div class="w-61 margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmLogradouroTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10"
              dataKey="TIPO_LOGRAD" @row-click="handleRowClick" :rowClass="rowClass">
          ...
          <div class="w-35">
            <div class="box-tables-info" v-if="selectedRow && !showAddBox">
              ...
              <input v-model="selectedRow.TIPO_LOGRAD" maxlength="5" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
              <p class="p-labels">Descrição</p>
              <input v-model="selectedRow.DESC_TIPO_LOGRAD" maxlength="15" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
            ...
            <div class="box-tables-info" v-if="showAddBox && !selectedRow">
              <p class="p-labels">Código</p>
              <input class="inputs margin-bottom-30" maxlength="5" v-model="addTipoLograd" />
              <p class="p-labels">Descrição</p>
              <input class="inputs margin-bottom-30" maxlength="15" v-model="addDescTipoLograd" />
              <div class="flex justify-center gap-4 mt-40">
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {
          name: "DESC_TIPO_LOGRAD",
          required: true,
          label: "Descrição",
          align: "left",
          field: "DESC_TIPO_LOGRAD",
        },
      ],
      frmLogradouroTable: []
    };
  ...
  methods: {
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.TIPO_LOGRAD === data.TIPO_LOGRAD
      };
    },
    ...
    async addToTable() {
      ...
      try {
        const response = await axios.post(`${baseUrl}/TIPO_LOGRADOURO_ADD`, {
          TIPO_LOGRAD: this.addTipoLograd,
          DESC_TIPO_LOGRAD: this.addDescTipoLograd,
    ...
    async fetchData() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.get(`${baseUrl}/TIPO_LOGRADOURO_ALL`);
        this.data = response.data;
        this.frmLogradouroTable = this.data;
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
      } finally {
    ...
    async deleteFromTable() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.delete(`${baseUrl}/TIPO_LOGRADOURO_DELETE/${this.selectedRow.TIPO_LOGRAD}/${user.username}`);
        console.log(response.data);
        await this.fetchData();
    ...
    async saveUpdate() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.post(`${baseUrl}/TIPO_LOGRADOURO_UPDATE`, {
          TIPO_LOGRAD: this.selectedRow.TIPO_LOGRAD,
          DESC_TIPO_LOGRAD: this.selectedRow.DESC_TIPO_LOGRAD,
          NOME_ATUALIZ: user.username
        });

        if (response.status === 200) {
          this.frmLogradouroTable = this.frmLogradouroTable.map(row =>
            row.TIPO_LOGRAD === this.selectedRow.TIPO_LOGRAD ? this.selectedRow : row
          );
          this.isEditing = false;
        } else {
          console.error("Erro ao atualizar o Tipo de Logradouro");
        }
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmLogradouroTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
      });
