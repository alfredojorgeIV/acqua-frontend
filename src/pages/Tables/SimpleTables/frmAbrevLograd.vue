    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Abreviação Código Logradouro</h1>
        </div>
        <div class="flex gap-4">
          <div class="w-61 margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmAbrevLogradTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10"
              dataKey="ABREV_COG" @row-click="handleRowClick" :rowClass="rowClass">
          ...
          <div class="w-35">
            <div class="box-tables-info" v-if="selectedRow && !showAddBox">
              ...
              <input v-model="selectedRow.ABREV_COG" maxlength="5" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
              <p class="p-labels">Descrição</p>
              <input v-model="selectedRow.DESC_ABREV_COG" maxlength="15" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
            ...
            <div class="box-tables-info" v-if="showAddBox && !selectedRow">
              <p class="p-labels">Código</p>
              <input class="inputs margin-bottom-30" maxlength="5" v-model="addAbrevCog" />
              <p class="p-labels">Descrição</p>
              <input class="inputs margin-bottom-30" maxlength="15" v-model="addDescAbrevCog" />
              <div class="flex justify-center gap-4 mt-40">
...
export default {
  ...
  methods: {
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.ABREV_COG === data.ABREV_COG
      };
    },
    ...
    async addToTable() {
      ...
      try {
        const response = await axios.post(`${baseUrl}/ABREV_COG_LOGRAD_ADD`, {
          ABREV_COG: this.addAbrevCog,
          DESC_ABREV_COG: this.addDescAbrevCog,
    ...
    async fetchData() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.get(`${baseUrl}/ABREV_COG_LOGRAD_ALL`);
        this.data = response.data;
        this.frmAbrevLogradTable = this.data;
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
      } finally {
    ...
    async deleteFromTable() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.delete(`${baseUrl}/ABREV_COG_LOGRAD_DELETE/${this.selectedRow.ABREV_COG}/${user.username}`);
        console.log(response.data);
        await this.fetchData();
    ...
    async saveUpdate() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.post(`${baseUrl}/ABREV_COG_LOGRAD_UPDATE`, {
          ABREV_COG: this.selectedRow.ABREV_COG,
          DESC_ABREV_COG: this.selectedRow.DESC_ABREV_COG,
          NOME_ATUALIZ: user.username
        });

        if (response.status === 200) {
          this.frmAbrevLogradTable = this.frmAbrevLogradTable.map(row =>
            row.ABREV_COG === this.selectedRow.ABREV_COG ? this.selectedRow : row
          );
          this.isEditing = false;
        } else {
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmAbrevLogradTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
      });
