      ...
      <div class="content">
        <section>
          <div class="intro-services-to-be-billed">
            <h1 ref="title" class="title-page">Tipo de Pavimento da Calçada</h1>
          </div>
          <div class="flex gap-4">
            <div class="w-61 margin-top-30">
              ...
              <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
                :value="frmTipoPavimentoCalcadaTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10"
                dataKey="TC_CODIGO" @row-click="handleRowClick" :rowClass="rowClass">
            ...
            <div class="w-35">
              <div class="box-tables-info" v-if="selectedRow && !showAddBox">
                ...
                <input v-model="selectedRow.TC_CODIGO" maxlength="5" class="inputs margin-bottom-30"
                  :disabled="!isEditing" />
                <p class="p-labels">Descrição</p>
                <input v-model="selectedRow.TC_DESCRICAO" maxlength="15" class="inputs margin-bottom-30"
                  :disabled="!isEditing" />
              ...
              <div class="box-tables-info" v-if="showAddBox && !selectedRow">
                <p class="p-labels">Código</p>
                <input class="inputs margin-bottom-30" maxlength="5" v-model="addTcCodigo" />
                <p class="p-labels">Descrição</p>
                <input class="inputs margin-bottom-30" maxlength="15" v-model="addTcDescricao" />
                <div class="flex justify-center gap-4 mt-40">
  ...
  export default {
    ...
    methods: {
      ...
      async addToTable() {
        ...
        try {
          const response = await axios.post(`${baseUrl}/TIPO_PAVIMENTO_CALCADA_ADD`, {
            TC_CODIGO: Number(this.addTcCodigo),
            TC_DESCRICAO: this.addTcDescricao,
      ...
      async fetchData() {
        ...
        const baseUrl = user.databaseUrl || 'http://localhost:3000';
        try {
          const response = await axios.get(`${baseUrl}/TIPO_PAVIMENTO_CALCADA_ALL`);
          this.data = response.data;
          this.frmTipoPavimentoCalcadaTable = this.data;
        } catch (error) {
          console.error('Erro:', error);
          this.error = error.toString();
        } finally {
      ...
      async deleteFromTable() {
        ...
        const baseUrl = user.databaseUrl || 'http://localhost:3000';
        try {
          const response = await axios.delete(`${baseUrl}/TIPO_PAVIMENTO_CALCADA_DELETE/${this.selectedRow.TC_CODIGO}/${user.username}`);
          console.log(response.data);
          await this.fetchData();
      ...
      async saveUpdate() {
        ...
        const baseUrl = user.databaseUrl || 'http://localhost:3000';
        try {
          const response = await axios.post(`${baseUrl}/TIPO_PAVIMENTO_CALCADA_UPDATE`, {
            TC_CODIGO: Number(this.selectedRow.TC_CODIGO),
            TC_DESCRICAO: this.selectedRow.TC_DESCRICAO,
            NOME_ATUALIZ: user.username
          });
  
          if (response.status === 200) {
            this.frmTipoPavimentoCalcadaTable = this.frmTipoPavimentoCalcadaTable.map(row =>
              row.TC_CODIGO === this.selectedRow.TC_CODIGO ? this.selectedRow : row
            );
            this.isEditing = false;
          } else {
            console.error("Erro ao atualizar o Tipo de Pavimento");
          }
        } catch (error) {
          console.error('Erro:', error);
          this.error = error.toString();
      ...
      printPDF() {
        ...
        doc.autoTable({
          ...
          body: this.frmTipoPavimentoCalcadaTable.map(row => this.columns.map(col => row[col.field])),
          theme: 'grid',
        });
