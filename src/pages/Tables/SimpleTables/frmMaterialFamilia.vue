      ...
      <div class="content">
        <section>
          <div class="intro-services-to-be-billed">
            <h1 ref="title" class="title-page">Tipo da Família do Material</h1>
          </div>
          <div class="flex gap-4">
            <div class="w-61 margin-top-30">
              ...
              <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
                :value="frmMaterialFamiliaTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10"
                dataKey="CODIGO" @row-click="handleRowClick" :rowClass="rowClass">
            ...
            <div class="w-35">
              <div class="box-tables-info" v-if="selectedRow && !showAddBox">
                ...
                <input v-model="selectedRow.CODIGO" maxlength="5" class="inputs margin-bottom-30"
                  :disabled="!isEditing" />
                <p class="p-labels">Descrição</p>
                <input v-model="selectedRow.DESCRICAO" maxlength="15" class="inputs margin-bottom-30"
                  :disabled="!isEditing" />
              ...
              <div class="box-tables-info" v-if="showAddBox && !selectedRow">
                <p class="p-labels">Código</p>
                <input class="inputs margin-bottom-30" maxlength="5" v-model="addCodigo" />
                <p class="p-labels">Descrição</p>
                <input class="inputs margin-bottom-30" maxlength="15" v-model="addDescricao" />
                <div class="flex justify-center gap-4 mt-40">
  ...
  export default {
    ...
    methods: {
      ...
      rowClass(data) {
        return {
          'pointer-cursor': true,
          'active-row': this.selectedRow && this.selectedRow.CODIGO === data.CODIGO
        };
      },
      ...
      async addToTable() {
        ...
        try {
          const response = await axios.post(`${baseUrl}/MATERIAL_FAMILIA_ADD`, {
            CODIGO: Number(this.addCodigo),
      ...
      async fetchData() {
        ...
        const baseUrl = user.databaseUrl || 'http://localhost:3000';
        try {
          const response = await axios.get(`${baseUrl}/MATERIAL_FAMILIA_ALL`);
          this.data = response.data;
          this.frmMaterialFamiliaTable = this.data;
        } catch (error) {
          console.error('Erro:', error);
          this.error = error.toString();
        } finally {
      ...
      async deleteFromTable() {
        ...
        const baseUrl = user.databaseUrl || 'http://localhost:3000';
        try {
          const response = await axios.delete(`${baseUrl}/MATERIAL_FAMILIA_DELETE/${this.selectedRow.CODIGO}/${user.username}`);
          console.log(response.data);
          await this.fetchData();
      ...
      async saveUpdate() {
        ...
        const baseUrl = user.databaseUrl || 'http://localhost:3000';
        try {
          const response = await axios.post(`${baseUrl}/MATERIAL_FAMILIA_UPDATE`, {
            CODIGO: Number(this.selectedRow.CODIGO),
            DESCRICAO: this.selectedRow.DESCRICAO,
            NOME_ATUALIZ: user.username
          });
  
          if (response.status === 200) {
            this.frmMaterialFamiliaTable = this.frmMaterialFamiliaTable.map(row =>
              row.CODIGO === this.selectedRow.CODIGO ? this.selectedRow : row
            );
            this.isEditing = false;
          } else {
            console.error("Erro ao atualizar o o Tipo de Família do Material");
          }
        } catch (error) {
          console.error('Erro:', error);
          this.error = error.toString();
      ...
      printPDF() {
        ...
        doc.autoTable({
          ...
          body: this.frmMaterialFamiliaTable.map(row => this.columns.map(col => row[col.field])),
          theme: 'grid',
        });
