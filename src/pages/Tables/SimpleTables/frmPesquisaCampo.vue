    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Pesquisa de Campo</h1>
        </div>
        <div class="flex gap-4">
          <div class="w-61 margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmPesquisaCampoTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10"
              dataKey="PC_CODIGO" @row-click="handleRowClick" :rowClass="rowClass">
          ...
          <div class="w-35">
            <div class="box-tables-info" v-if="selectedRow && !showAddBox">
              ...
              <input v-model="selectedRow.PC_CODIGO" maxlength="5" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
              <p class="p-labels">Descrição</p>
              <input v-model="selectedRow.PC_DESCRICAO" maxlength="15" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
            ...
            <div class="box-tables-info" v-if="showAddBox && !selectedRow">
              <p class="p-labels">Código</p>
              <input class="inputs margin-bottom-30" maxlength="5" v-model="addPcCodigo" />
              <p class="p-labels">Descrição</p>
              <input class="inputs margin-bottom-30" maxlength="15" v-model="addPcDescricao" />
              <div class="flex justify-center gap-4 mt-40">
...
export default {
  ...
  methods: {
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.PC_CODIGO === data.PC_CODIGO
      };
    },
    ...
    async addToTable() {
      ...
      try {
        const response = await axios.post(`${baseUrl}/PESQUISA_CAMPO_ADD`, {
          PC_CODIGO: Number(this.addPcCodigo),
          PC_DESCRICAO: this.addPcDescricao,
    ...
    async fetchData() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.get(`${baseUrl}/PESQUISA_CAMPO_ALL`);
        this.data = response.data;
        this.frmPesquisaCampoTable = this.data;
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
      } finally {
    ...
    async deleteFromTable() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.delete(`${baseUrl}/PESQUISA_CAMPO_DELETE/${this.selectedRow.PC_CODIGO}/${user.username}`);
        console.log(response.data);
        await this.fetchData();
    ...
    async saveUpdate() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.post(`${baseUrl}/PESQUISA_CAMPO_UPDATE`, {
          PC_CODIGO: Number(this.selectedRow.PC_CODIGO),
          PC_DESCRICAO: this.selectedRow.PC_DESCRICAO,
          NOME_ATUALIZ: user.username
        });

        if (response.status === 200) {
          this.frmPesquisaCampoTable = this.frmPesquisaCampoTable.map(row =>
            row.PC_CODIGO === this.selectedRow.PC_CODIGO ? this.selectedRow : row
          );
          this.isEditing = false;
        } else {
          console.error("Erro ao atualizar o Pesquisa de Campo");
        }
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmPesquisaCampoTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
      });
