    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 class="title-page">Tipo de cotação</h1>
        </div>
        <div class="flex gap-4">
          <div class="w-61 margin-top-30">
            ...
            <DataTable style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="quotationTypeTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10" dataKey="id"
          ...
          <div class="w-35">
            <div class="box-tables-info" v-if="selectedRow && !showAddBox">
              ...
              <input v-model="selectedRow.TIPO_COT" class="inputs margin-bottom-30" :disabled="!isEditing" />
              <p class="p-labels">Descrição</p>
              <input v-model="selectedRow.DESC_COTACAO" class="inputs margin-bottom-30" :disabled="!isEditing" />
            ...
            <div class="box-tables-info" v-if="showAddBox && !selectedRow">
              <p class="p-labels">Código</p>
              <input class="inputs margin-bottom-30" v-model="addTIPO_COT" />
              <p class="p-labels">Descrição</p>
              <input class="inputs margin-bottom-30" v-model="addDescricao" />

              <div class="flex justify-center gap-4 mt-40">
                <Button class="primary-button flex align-center" @click="addTIPO_COTTarifa">
...
export default {
  ...
  methods: {
    ...
    async addTIPO_COTTarifa() {
      this.isLoading = true;
      try {
        const response = await axios.post('http://localhost:3000/TIPO_COTACAO_ADD', {
          TIPO_COT: Number(this.addTIPO_COT), // Corrected
          DESC_COTACAO: this.addDescricao,
          NOME_ATUALIZ: "Luiza"
        });
    ...
    async fetchData() {
      this.isLoading = true;
      try {
        const response = await axios.get('http://localhost:3000/TIPO_COTACAO_ALL');
        this.data = response.data;
        this.quotationTypeTable = this.data;
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
      } finally {
    ...
    async deleteTaxCategory() {
      this.isLoading = true;
      try {
        const response = await axios.delete(`http://localhost:3000/TIPO_COTACAO_DELETE/${this.selectedRow.TIPO_COT}`);
        console.log(response.data);
        await this.fetchData();
    ...
    async saveUpdate() {
      this.isLoading = true;
      try {
        const response = await axios.post(`http://localhost:3000/TIPO_COTACAO_UPDATE`, {
          TIPO_COT: this.selectedRow.TIPO_COT,
          DESC_COTACAO: this.selectedRow.DESC_COTACAO,
          NOME_ATUALIZ: "Luiza"
        });

        if (response.status === 200) {
          this.quotationTypeTable = this.quotationTypeTable.map(row => row.TIPO_COT === this.selectedRow.TIPO_COT ? this.selectedRow : row);
          this.isEditing = false;
        } else {
          console.error("Erro ao atualizar a TIPO_COT de tarifa");
        }
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
