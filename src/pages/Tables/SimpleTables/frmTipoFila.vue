          ...
          <div class="flex gap-4">
            ...
            <div class="w-35">
              <div class="box-tables-info" v-if="selectedRow && !showAddBox">
                <p class="p-labels">Fila</p>
                <input
                  v-model="selectedRow.TF_FILA"
                  maxlength="5"
                ...
                <input
                  v-model="selectedRow.TF_DESCRICAO"
                  maxlength="15"
                  class="inputs margin-bottom-30"
                  :disabled="!isEditing"
                />
    ...
    methods: {
      ...
      rowClass(data) {
        return {
          "pointer-cursor": true,
          "active-row":
            this.selectedRow && this.selectedRow.TF_FILA === data.TF_FILA,
        };
      },
      ...
      async addToTable() {
        ...
        try {
          const response = await axios.post(`${baseUrl}/TIPO_FILA_ADD`, {
            TF_FILA: Number(this.addTfFila),
            TF_DESCRICAO: this.addTfDescricao,
            TF_CODIGO: this.addTfCodigo,
      ...
      async fetchData() {
        ...
        const baseUrl = user.databaseUrl || "http://localhost:3000";
        try {
          const response = await axios.get(`${baseUrl}/TIPO_FILA_ALL`);
          this.data = response.data;
          this.frmTipoFilaTable = this.data;
        } catch (error) {
          console.error("Erro:", error);
          this.error = error.toString();
        } finally {
      ...
      async deleteFromTable() {
        ...
        try {
          const response = await axios.delete(
            `${baseUrl}/TIPO_FILA_DELETE/${this.selectedRow.TF_FILA}/${user.username}`
          );
          console.log(response.data);
          await this.fetchData();
      ...
      async saveUpdate() {
        ...
        const baseUrl = user.databaseUrl || "http://localhost:3000";
        try {
          const response = await axios.post(`${baseUrl}/TIPO_FILA_UPDATE`, {
            TF_FILA: Number(this.selectedRow.TF_FILA),
            TF_DESCRICAO: this.selectedRow.TF_DESCRICAO,
            TF_CODIGO: this.selectedRow.TF_CODIGO,
            NOME_ATUALIZ: user.username,
          });
  
          if (response.status === 200) {
            this.frmTipoFilaTable = this.frmTipoFilaTable.map((row) =>
              row.TF_FILA === this.selectedRow.TF_FILA
                ? this.selectedRow
                : row
            );
            this.isEditing = false;
      ...
      printPDF() {
        ...
        doc.autoTable({
          ...
          body: this.frmTipoFilaTable.map((row) =>
