    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Tipo Unidade Material</h1>
        </div>
        <div class="flex gap-4">
          <div class="w-61 margin-top-30">
            ...
            <DataTable
              ...
              "
              :value="frmTipoUnidMaterialTable"
          ...
          <div class="w-35">
            <div class="box-tables-info" v-if="selectedRow && !showAddBox">
              ...
              <input
                v-model="selectedRow.TM_DESCRICAO"
                maxlength="15"
                class="inputs margin-bottom-30"
                :disabled="!isEditing"
              />
              <p class="p-labels">Descrição abreviada</p>
              <input
                v-model="selectedRow.TM_DESC_ABREV"
                maxlength="15"
                class="inputs margin-bottom-30"
                :disabled="!isEditing"
              />
            ...
            <div class="box-tables-info" v-if="showAddBox && !selectedRow">
              ...
              <input
                class="inputs margin-bottom-30"
                maxlength="15"
                v-model="addTmDescricao"
              />
              <p class="p-labels">Descrição abreviada</p>
              <input
                class="inputs margin-bottom-30"
                maxlength="15"
                v-model="addTfDescAbrev"
              />
              <div class="flex justify-center gap-4 mt-40">
                <Button
  ...
  methods: {
    ...
    rowClass(data) {
      return {
        "pointer-cursor": true,
        "active-row":
          this.selectedRow && this.selectedRow.TM_CODIGO === data.TM_CODIGO,
      };
    },
    ...
    async addToTable() {
      ...
      try {
        const response = await axios.post(`${baseUrl}/TIPO_UNID_MATERIAL_ADD`, {
          TM_CODIGO: Number(this.addTmCOdigo),
          TM_DESCRICAO: this.addTmDescricao,
          TM_DESC_ABREV: this.addTfDescAbrev,
    ...
    async fetchData() {
      ...
      const baseUrl = user.databaseUrl || "http://localhost:3000";
      try {
        const response = await axios.get(`${baseUrl}/TIPO_UNID_MATERIAL_ALL`);
        this.data = response.data;
        this.frmTipoUnidMaterialTable = this.data;
      } catch (error) {
        console.error("Erro:", error);
        this.error = error.toString();
      } finally {
    ...
    async deleteFromTable() {
      ...
      try {
        const response = await axios.delete(
          `${baseUrl}/TIPO_UNID_MATERIAL_DELETE/${this.selectedRow.TM_CODIGO}/${user.username}`
        );
        console.log(response.data);
        await this.fetchData();
    ...
    async saveUpdate() {
      ...
      try {
        const response = await axios.post(
          `${baseUrl}/TIPO_UNID_MATERIAL_UPDATE`,
          {
            TM_CODIGO: Number(this.selectedRow.TM_CODIGO),
            TM_DESCRICAO: this.selectedRow.TM_DESCRICAO,
            TM_DESC_ABREV: this.selectedRow.TM_DESC_ABREV,
            NOME_ATUALIZ: user.username,
          }
        );

        if (response.status === 200) {
          this.frmTipoUnidMaterialTable = this.frmTipoUnidMaterialTable.map(
            (row) =>
              row.TM_CODIGO === this.selectedRow.TM_CODIGO
                ? this.selectedRow
                : row
          );
          this.isEditing = false;
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmTipoUnidMaterialTable.map((row) =>
