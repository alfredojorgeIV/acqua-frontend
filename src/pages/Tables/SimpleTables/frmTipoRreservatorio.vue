      ...
      <div class="content">
        <section>
          <div class="intro-services-to-be-billed">
            <h1 ref="title" class="title-page">Tipo de Reservatório</h1>
          </div>
          <div class="flex gap-4">
            <div class="w-61 margin-top-30">
              ...
              <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
                :value="frmTipoReservatorioTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10"
                dataKey="TR_CODIGO" @row-click="handleRowClick" :rowClass="rowClass">
            ...
            <div class="w-35">
              <div class="box-tables-info" v-if="selectedRow && !showAddBox">
                ...
                <input v-model="selectedRow.TR_CODIGO" maxlength="5" class="inputs margin-bottom-30"
                  :disabled="!isEditing" />
                <p class="p-labels">Descrição</p>
                <input v-model="selectedRow.TR_DESCRICAO" maxlength="15" class="inputs margin-bottom-30"
                  :disabled="!isEditing" />
              ...
              <div class="box-tables-info" v-if="showAddBox && !selectedRow">
                <p class="p-labels">Código</p>
                <input class="inputs margin-bottom-30" maxlength="5" v-model="addTrCodigo" />
                <p class="p-labels">Descrição</p>
                <input class="inputs margin-bottom-30" maxlength="15" v-model="addTrDescricao" />
                <div class="flex justify-center gap-4 mt-40">
  ...
  export default {
    ...
    methods: {
      ...
      rowClass(data) {
        return {
          'pointer-cursor': true,
          'active-row': this.selectedRow && this.selectedRow.TR_CODIGO === data.TR_CODIGO
        };
      },
      ...
      async addToTable() {
        ...
        try {
          const response = await axios.post(`${baseUrl}/TIPO_RESERVATORIO_ADD`, {
            TR_CODIGO: Number(this.addTrCodigo),
            TR_DESCRICAO: this.addTrDescricao,
      ...
      async fetchData() {
        ...
        const baseUrl = user.databaseUrl || 'http://localhost:3000';
        try {
          const response = await axios.get(`${baseUrl}/TIPO_RESERVATORIO_ALL`);
          this.data = response.data;
          this.frmTipoReservatorioTable = this.data;
        } catch (error) {
          console.error('Erro:', error);
          this.error = error.toString();
        } finally {
      ...
      async deleteFromTable() {
        ...
        const baseUrl = user.databaseUrl || 'http://localhost:3000';
        try {
          const response = await axios.delete(`${baseUrl}/TIPO_RESERVATORIO_DELETE/${this.selectedRow.TR_CODIGO}/${user.username}`);
          console.log(response.data);
          await this.fetchData();
      ...
      async saveUpdate() {
        ...
        const baseUrl = user.databaseUrl || 'http://localhost:3000';
        try {
          const response = await axios.post(`${baseUrl}/TIPO_RESERVATORIO_UPDATE`, {
            TR_CODIGO: Number(this.selectedRow.TR_CODIGO),
            TR_DESCRICAO: this.selectedRow.TR_DESCRICAO,
            NOME_ATUALIZ: user.username
          });
  
          if (response.status === 200) {
            this.frmTipoReservatorioTable = this.frmTipoReservatorioTable.map(row =>
              row.TR_CODIGO === this.selectedRow.TR_CODIGO ? this.selectedRow : row
            );
            this.isEditing = false;
          } else {
            console.error("Erro ao atualizar o Tipo do Pavimento da Rua");
          }
        } catch (error) {
          console.error('Erro:', error);
          this.error = error.toString();
      ...
      printPDF() {
        ...
        doc.autoTable({
          ...
          body: this.frmTipoReservatorioTable.map(row => this.columns.map(col => row[col.field])),
          theme: 'grid',
        });
