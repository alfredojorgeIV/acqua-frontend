        ...
        <div class="flex gap-4">
          ...
          <div class="w-35">
            <div class="box-tables-info" v-if="selectedRow && !showAddBox">
              ...
              <input v-model="selectedRow.DESC_REDUZIDA" class="inputs margin-bottom-30" :disabled="!isEditing" />
            ...
            <div class="box-tables-info" v-if="showAddBox && !selectedRow">
              ...
              <p class="p-labels">Descrição reduzida</p>
              <input class="inputs margin-bottom-30" v-model="addDescReduzida" />
    ...
    async saveUpdate() {
      ...
      try {
        const response = await axios.post(`http://localhost:3000/CATEGORIA_TARIFA_UPDATE`, {
          ...
          DESC_REDUZIDA: this.selectedRow.DESC_REDUZIDA,
          NOME_ATUALIZ: "Luiza"
        });

        if (response.status === 200) {
          this.taxCategoryTable = this.taxCategoryTable.map(row => row.CATEGORIA === this.selectedRow.CATEGORIA ? this.selectedRow : row);
          this.isEditing = false;
        } else {
