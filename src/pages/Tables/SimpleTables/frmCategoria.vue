    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Categoria</h1>
        </div>
        <div class="flex gap-4">
          <div class="w-61 margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmCategoriaTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10"
              dataKey="CATEGORIA" @row-click="handleRowClick" :rowClass="rowClass">
          ...
          <div class="w-35">
            <div class="box-tables-info" v-if="selectedRow && !showAddBox">
              ...
              <input v-model="selectedRow.CATEGORIA" maxlength="5" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
              <p class="p-labels">Descrição</p>
              <input v-model="selectedRow.DESC_CATEGORIA" maxlength="15" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
            ...
            <div class="box-tables-info" v-if="showAddBox && !selectedRow">
              <p class="p-labels">Código</p>
              <input class="inputs margin-bottom-30" maxlength="5" v-model="addCategoria" />
              <p class="p-labels">Descrição</p>
              <input class="inputs margin-bottom-30" maxlength="15" v-model="addDescCategoria" />
              <div class="flex justify-center gap-4 mt-40">
...
export default {
  ...
  methods: {
    ...
    async addToTable() {
      ...
      try {
        const response = await axios.post(`${baseUrl}/CATEGORIA_ADD`, {
          CATEGORIA: Number(this.addCategoria),
          DESC_CATEGORIA: this.addDescCategoria,
    ...
    async fetchData() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.get(`${baseUrl}/CATEGORIA_ALL`);
        this.data = response.data;
        this.frmCategoriaTable = this.data;
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
      } finally {
    ...
    async deleteFromTable() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.delete(`${baseUrl}/CATEGORIA_DELETE/${this.selectedRow.CATEGORIA}/${user.username}`);
        console.log(response.data);
        await this.fetchData();
    ...
    async saveUpdate() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.post(`${baseUrl}/CATEGORIA_UPDATE`, {
          CATEGORIA: Number(this.selectedRow.CATEGORIA),
          DESC_CATEGORIA: this.selectedRow.DESC_CATEGORIA,
          NOME_ATUALIZ: user.username
        });

        if (response.status === 200) {
          this.frmCategoriaTable = this.frmCategoriaTable.map(row =>
            row.CATEGORIA === this.selectedRow.CATEGORIA ? this.selectedRow : row
          );
          this.isEditing = false;
        } else {
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmCategoriaTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
      });
