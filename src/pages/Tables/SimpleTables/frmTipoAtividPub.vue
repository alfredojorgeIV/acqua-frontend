    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Atividade Pública</h1>
        </div>
        <div class="flex gap-4">
          <div class="w-61 margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmTipoAtividPubTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10"
              dataKey="ATIVIDADE" @row-click="handleRowClick" :rowClass="rowClass">
          ...
          <div class="w-35">
            <div class="box-tables-info" v-if="selectedRow && !showAddBox">
              ...
              <input v-model="selectedRow.ATIVIDADE" maxlength="5" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
              <p class="p-labels">Descrição</p>
              <input v-model="selectedRow.DESC_ATIVIDADE" maxlength="15" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
            ...
            <div class="box-tables-info" v-if="showAddBox && !selectedRow">
              <p class="p-labels">Código</p>
              <input class="inputs margin-bottom-30" maxlength="5" v-model="addAtividade" />
              <p class="p-labels">Descrição</p>
              <input class="inputs margin-bottom-30" maxlength="15" v-model="addDescAtividade" />
              <div class="flex justify-center gap-4 mt-40">
...
export default {
  ...
  methods: {
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.ATIVIDADE === data.ATIVIDADE
      };
    },
    ...
    async addToTable() {
      ...
      try {
        const response = await axios.post(`${baseUrl}/TIPO_ATIVIDADE_PUB_ADD`, {
          ATIVIDADE: Number(this.addAtividade),
          DESC_ATIVIDADE: this.addDescAtividade,
    ...
    async fetchData() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.get(`${baseUrl}/TIPO_ATIVIDADE_PUB_ALL`);
        this.data = response.data;
        this.frmTipoAtividPubTable = this.data;
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
      } finally {
    ...
    async deleteFromTable() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.delete(`${baseUrl}/TIPO_ATIVIDADE_PUB_DELETE/${this.selectedRow.ATIVIDADE}/${user.username}`);
        console.log(response.data);
        await this.fetchData();
    ...
    async saveUpdate() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.post(`${baseUrl}/TIPO_ATIVIDADE_PUB_UPDATE`, {
          ATIVIDADE: Number(this.selectedRow.ATIVIDADE),
          DESC_ATIVIDADE: this.selectedRow.DESC_ATIVIDADE,
          NOME_ATUALIZ: user.username
        });

        if (response.status === 200) {
          this.frmTipoAtividPubTable = this.frmTipoAtividPubTable.map(row =>
            row.ATIVIDADE === this.selectedRow.ATIVIDADE ? this.selectedRow : row
          );
          this.isEditing = false;
        } else {
          console.error("Erro ao atualizar o Atividade Pública");
        }
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmTipoAtividPubTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
      });
