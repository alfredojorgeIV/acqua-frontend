    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Situação do Imóvel</h1>
        </div>
        <div class="flex gap-4">
          <div class="w-61 margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmSitImovelTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10"
              dataKey="SITUACAO_IMOVEL" @row-click="handleRowClick" :rowClass="rowClass">
          ...
          <div class="w-35">
            <div class="box-tables-info" v-if="selectedRow && !showAddBox">
              ...
              <input v-model="selectedRow.SITUACAO_IMOVEL" maxlength="5" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
              <p class="p-labels">Descrição</p>
              <input v-model="selectedRow.DESC_SITUACAO" maxlength="15" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
            ...
            <div class="box-tables-info" v-if="showAddBox && !selectedRow">
              <p class="p-labels">Código</p>
              <input class="inputs margin-bottom-30" maxlength="5" v-model="addSituacaoImovel" />
              <p class="p-labels">Descrição</p>
              <input class="inputs margin-bottom-30" maxlength="15" v-model="addDescSituacao" />
              <div class="flex justify-center gap-4 mt-40">
...
export default {
  ...
  methods: {
    handleAddRowClick(event) {
      ...
      this.addSituacaoImovel = '';
      this.addDescSituacao = '';
    },
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.SITUACAO_IMOVEL === data.SITUACAO_IMOVEL
      };
    },
    ...
    async addToTable() {
      ...
      try {
        const response = await axios.post(`${baseUrl}/SITUACAO_IMOVEL_ADD`, {
          SITUACAO_IMOVEL: Number(this.addSituacaoImovel),
          DESC_SITUACAO: this.addDescSituacao,
    ...
    async fetchData() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.get(`${baseUrl}/SITUACAO_IMOVEL_ALL`);
        this.data = response.data;
        this.frmSitImovelTable = this.data;
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
      } finally {
    ...
    async deleteFromTable() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.delete(`${baseUrl}/SITUACAO_IMOVEL_DELETE/${this.selectedRow.SITUACAO_IMOVEL}/${user.username}`);
        console.log(response.data);
        await this.fetchData();
    ...
    async saveUpdate() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.post(`${baseUrl}/SITUACAO_IMOVEL_UPDATE`, {
          SITUACAO_IMOVEL: Number(this.selectedRow.SITUACAO_IMOVEL),
          DESC_SITUACAO: this.selectedRow.DESC_SITUACAO,
          NOME_ATUALIZ: user.username
        });

        if (response.status === 200) {
          this.frmSitImovelTable = this.frmSitImovelTable.map(row =>
            row.SITUACAO_IMOVEL === this.selectedRow.SITUACAO_IMOVEL ? this.selectedRow : row
          );
          this.isEditing = false;
        } else {
          console.error("Erro ao atualizar o Situação do Imóvel");
        }
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmSitImovelTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
      });
