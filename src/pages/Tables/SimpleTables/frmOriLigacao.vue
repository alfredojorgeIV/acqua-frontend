    ...
    <div class="content">
      <section>
        <div class="intro-services-to-be-billed">
          <h1 ref="title" class="title-page">Origem da Ordem de Serviço</h1>
        </div>
        <div class="flex gap-4">
          <div class="w-61 margin-top-30">
            ...
            <DataTable ref="dataTable" style="border-bottom-left-radius: 10px; border-bottom-right-radius: 10px !important;"
              :value="frmOriLigacaoTable" tableStyle="min-width: 50rem" :paginator="true" :rows="10"
              dataKey="ORIG_LIGACAO" @row-click="handleRowClick" :rowClass="rowClass">
          ...
          <div class="w-35">
            <div class="box-tables-info" v-if="selectedRow && !showAddBox">
              ...
              <input v-model="selectedRow.ORIG_LIGACAO" maxlength="5" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
              <p class="p-labels">Descrição</p>
              <input v-model="selectedRow.DESC_ORIG_LIGACAO" maxlength="15" class="inputs margin-bottom-30"
                :disabled="!isEditing" />
            ...
            <div class="box-tables-info" v-if="showAddBox && !selectedRow">
              <p class="p-labels">Código</p>
              <input class="inputs margin-bottom-30" maxlength="5" v-model="addOrigLigacao" />
              <p class="p-labels">Descrição</p>
              <input class="inputs margin-bottom-30" maxlength="15" v-model="addDescOrigLigacao" />
              <div class="flex justify-center gap-4 mt-40">
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {
          name: "DESC_ORIG_LIGACAO",
          required: true,
          label: "Descrição",
          align: "left",
          field: "DESC_ORIG_LIGACAO",
        },
      ],
      frmOriLigacaoTable: []
    };
  ...
  methods: {
    ...
    rowClass(data) {
      return {
        'pointer-cursor': true,
        'active-row': this.selectedRow && this.selectedRow.ORIG_LIGACAO === data.ORIG_LIGACAO
      };
    },
    ...
    async addToTable() {
      ...
      try {
        const response = await axios.post(`${baseUrl}/ORIG_LIGACAO_ADD`, {
          ORIG_LIGACAO: Number(this.addOrigLigacao),
          DESC_ORIG_LIGACAO: this.addDescOrigLigacao,
    ...
    async fetchData() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.get(`${baseUrl}/ORIG_LIGACAO_ALL`);
        this.data = response.data;
        this.frmOriLigacaoTable = this.data;
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
      } finally {
    ...
    async deleteFromTable() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.delete(`${baseUrl}/ORIG_LIGACAO_DELETE/${this.selectedRow.ORIG_LIGACAO}/${user.username}`);
        console.log(response.data);
        await this.fetchData();
    ...
    async saveUpdate() {
      ...
      const baseUrl = user.databaseUrl || 'http://localhost:3000';
      try {
        const response = await axios.post(`${baseUrl}/ORIG_LIGACAO_UPDATE`, {
          ORIG_LIGACAO: Number(this.selectedRow.ORIG_LIGACAO),
          DESC_ORIG_LIGACAO: this.selectedRow.DESC_ORIG_LIGACAO,
          NOME_ATUALIZ: user.username
        });

        if (response.status === 200) {
          this.frmOriLigacaoTable = this.frmOriLigacaoTable.map(row =>
            row.ORIG_LIGACAO === this.selectedRow.ORIG_LIGACAO ? this.selectedRow : row
          );
          this.isEditing = false;
        } else {
          console.error("Erro ao atualizar o Origem da Ordem de Serviço");
        }
      } catch (error) {
        console.error('Erro:', error);
        this.error = error.toString();
    ...
    printPDF() {
      ...
      doc.autoTable({
        ...
        body: this.frmOriLigacaoTable.map(row => this.columns.map(col => row[col.field])),
        theme: 'grid',
      });
